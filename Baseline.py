...
class Baseline:
    def __init__(self):
        r(
            "suppressWarnings(suppressMessages(library(baseline))); getBaseline<-function(eic){  return(baseline(eic, method='medianWindow', hwm=20)@baseline[1,])  };"
        )
...
if __name__ == "__main__":
    ...
    CP = chromPeakPicking.MassSpecWavelet.MassSpecWavelet(
    ...
    if True:
        chromatogram.parse_file(
            "C:/PyMetExtract/implTest/D-GEN_Warth/GEN_24h_5uM_supern_3_neg_079.mzXML"
        )
        mz = 370.90112
        # mz = 283.06069
        ppm = 10.0
    ...
    ax.plot(
        [t / 60.0 for t in times], [-max(0, eic[i] - eicBL[i]) for i in range(len(eic))]
    )
