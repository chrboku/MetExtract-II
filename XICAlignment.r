...
alignPeaks<-function(eics, refTimes, peaks, ncol, align=TRUE, npoly=2, ra=c(1000:1200), maxGroupDist=22){
  mdat<-matrix(eics, ncol=ncol)
  len<-dim(mdat)[1]
  ...
  if(align){
    for(i in 1:ncol){
      ...
      for(peak in mpeaks[,i]){
        ...
        allpeaks[k]=refTimes[which.min(abs(w$warp.fun-peak[1]))]
        #allpeaks[k]=peak[1]
        k=k+1
      }
    }
