...
class SGRGenerator:
    ...
    def secondGoldenRule(self, useAtoms, curAtoms):
        ...
        return (
            (not (bool(valences % 2)) or not (bool(atomsWithOddValences % 2)))
            and (valences >= (2 * maxVal))
            and (valences >= (2 * (sum(curAtoms) - 1)))
        )
        ...
        if "C" in useAtoms:
            ...
            for i in range(0, len(useAtoms)):
                if useAtoms[i] in lookup.keys() and useAtoms[i] not in fixed:
                    atomsRange[i] = [
                        int(cCount * lookup[useAtoms[i]][0]),
                        int(cCount * lookup[useAtoms[i]][1] + 1),
                    ]
    ...
    def perm(
        ...
        for c in range(max(0, atomsRange[i][0]), atomsRange[i][1] + 1):
            ...
            if useSevenGoldenRules and useAtoms[i] == "C":
                useAtoms, atomsRange, curAtoms = self.goldenRuleGeneric(
                    useAtoms, atomsRange, i, curAtoms, fixed
                )
                useAtoms, atomsRange = self.sixthGoldenRule(
                    useAtoms, atomsRange, curAtoms, fixed
                )
                pass
            curAtoms[i] = c
            t = curMass + self.atoms[useAtoms[i]][0] * c

            if abs(t - molMass) < massDiff and (
                not useSevenGoldenRules
                or (self.secondGoldenRule(useAtoms, curAtoms) or not (useSecondRule))
            ):
                ret.append(
                    "".join(
                        [
                            str(useAtoms[j])
                            + str(curAtoms[j] > 1 and curAtoms[j] or "")
                            for j in range(0, len(useAtoms))
                            if curAtoms[j] != 0
                        ]
                    )
    ...
    def firstGoldenRule(self, molMass, useAtoms, atomsRange, fixed):
        ...
        swi = {
            "C": lambda y: y < 500 and 39 or (y < 1000 and 78 or 162),
    ...
    def findFormulas(
        ...
        if len(atomsRange) != len(useAtoms):
            raise Exception(
                "useAtoms and atomsRange parameter are not consistent (have a different length)"
            )
        cAtomsRange = []
        ...
        if useSevenGoldenRules:
            atomsRange = self.firstGoldenRule(
                molMass, useAtoms, atomsRange, fixed=fixed
            )

        res = self.perm(
            molMass,
            molMass * (ppm / 1000000.0),
            useAtoms,
            atomsRange,
            curAtoms=[t[0] for t in atomsRange],
            fixed=fixed,
            useSevenGoldenRules=useSevenGoldenRules,
            useSecondRule=useSecondRule,
        )
...
if __name__ == "__main__":
    ...
    for add in adducts:
        ...
        formsCRes = sfg.findFormulas(
            m,
            useAtoms=["C", "N", "H", "O", "P", "S"],
            atomsRange=[(1, 32), (0, 500), (0, 10000), (0, 400), [0, 10], [0, 20]],
            useSevenGoldenRules=True,
            useSecondRule=True,
            ppm=5.0,
        )
        ...
        for f in formsCRes:
            ...
            bs.append(
                Bunch(
                    ...
                    diffPPM=(fT.calcMolWeight(elems) - m) * 1000000.0 / m,
                    diffPPMAt300=(fT.calcMolWeight(elems) - m) * 1000000.0 / 300,
                )
            )
