...
class ProgressWrapper(QtWidgets.QDialog):
    ...
    def __init__(
        ...
        if showProgressBars:
            ...
            p.setStyleSheet(
                "QScrollArea { border-width: 0px;border-style: solid;border-color: rgb(170, 170, 170);}"
            )
        ...
        if showIndProgress:
            ...
            # p.setMaximumHeight(min(900, 35*len(indGroups))-(10 if len(indGroups)>1 else 0))
            p.setMinimumHeight(
                min(200, 35 * len(indGroups)) - (10 if len(indGroups) > 1 else 0)
            )
...
def callBack(a="hello", b="world", qt=None):
    if (
        QtWidgets.QMessageBox.question(
            qt, a + b, "Close", QtWidgets.QMessageBox.Yes | QtWidgets.QMessageBox.No
        )
...
if __name__ == "__main__":
    ...
    if False:
        pw = ProgressWrapper(
            1, showProgressBars=True, showLog=True, showIndProgress=False
        )
        pw.show()
