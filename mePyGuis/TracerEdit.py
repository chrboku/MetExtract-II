...
class ConfiguredTracer:
    ...
    def __str__(self):
        return (
            "ConfiguredTracer: %s %s %s %d  enrichment: %.3f %.3f amount %.1f %.1f monoisotopicRatio %.3f (check ratio %s) bias: %.1f %.1f"
            % (
                self.name,
                self.isotopeA,
                self.isotopeB,
...
class tracerEdit(QtWidgets.QDialog, Ui_Dialog):
    ...
    def updateTracer(self):
        ratio = (
            getRatio(self.tNativeEnrichment.value() / 100.0, self.tAtomCount.value(), 0)
            * self.tAmountA.value()
            / (
                getRatio(
                    self.tLabeledEnrichment.value() / 100.0, self.tAtomCount.value(), 0
                )
                * self.tAmountB.value()
            )
        )
    ...
    def setTracer(self, tracer):
        ...
        self.mzDelta = (
            getIsotopeMass(self.configuredTracer.isotopeB)[0]
            - getIsotopeMass(self.configuredTracer.isotopeA)[0]
        )
        self.updateTracer()
