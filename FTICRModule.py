...
class FTICRModuleWindow(QtWidgets.QMainWindow, Ui_MainWindow):
    def __init__(self, parent=None, initDir=None):
        ...
        self.pl.mpl_toolbar = NavigationToolbar(
            self.pl.canvas, self.result_MassSpectrum
        )
    ...
    def loadSamplesTSV(self):
        dialog = TSVLoaderEdit(
            mapping={"mz": "MZ", "Intensity": "Int"}, order=["mz", "Intensity"]
        )
        ...
        if dialog.executeDialog() == QtWidgets.QDialog.Accepted:
            ...
            for filename in dialog.getUserSelectedFile():
                ...
                msScan = self.fticr.importMSScan(
                    filename,
                    colNameMZ=mappingInfo["mz"],
    ...
    def processSamples(self):
        ...
        if len(msScans) == 0:
            QtWidgets.QMessageBox.warning(
                self,
                "MetExtract - FT-ICR modul",
                "Error: FT-ICR spectra are required for processing. Please laod some samples and then rerun the analysis",
        ...
        if saveFile is None or saveFile == "":
            QtWidgets.QMessageBox.warning(
                self,
                "MetExtract - FT-ICR modul",
                "Error: Results must be saved to a file. Please rerun the analysis and save it to a file",
                QtWidgets.QMessageBox.Ok,
            )
            return
        ...
        for elem in ["C", "H", "N", "O", "S", "P"]:
            if elem in elemsMax.keys():
                atoms.append(elem)
                atomsRange.append(
                    (elemsMin[elem] if elem in elemsMin.keys() else 0, elemsMax[elem])
                )
        ...
        foundSFs, averageMZErrors = self.fticr.processMSScans(
            ...
            checkIsotopologPattern=self.checkIsotopologPattern.isChecked(),
            intensityThreshold=self.intensityThreshold.value(),
            searchCAtomsRange=searchCAtomsRange,
            ppm=matchPPM,
            ...
            adducts=adducts,
            useSevenGoldenRules=self.useSevenGoldenRules.checkState()
            == QtCore.Qt.Checked,
            recalibrateWithUniqueSumFormulas=self.calibrate.checkState()
            == QtCore.Qt.Checked,
            dbs=self.dbs,
            setFunctionMax=pw.getCallingFunction()("max"),
            setFunctionValue=pw.getCallingFunction()("value"),
            setFunctionText=pw.getCallingFunction()("text"),
        )
        logging.info(
        ...
        for file in sorted(msScans.keys()):
            item2 = QtWidgets.QTreeWidgetItem(
                [
                    file,
                    "%.3f" % (averageMZErrors[file])
                    if file in averageMZErrors.keys()
                    else "-",
                ]
            )
        ...
        for i, re in enumerate(sorted(foundSFs, key=lambda sf: sf.meanMZ)):
            ...
            item = QtWidgets.QTreeWidgetItem(
                ["%.5f" % (re.meanMZ), "%d" % (re.cn), sfShow]
            )
            item.data = re

            for sf in re.sfs:
                ## re.sfs=[Bunch(sf=t, ion="[M+Cl]-", theoMZ=mz, ppmError=ppmError), ...]
                itemKid = QtWidgets.QTreeWidgetItem(
                    [sf.ion, sf.sf, "%.3f" % sf.ppmError]
                )
                itemKid.data = re
    ...
    def clearPlot(self, twinxs=1, shareX=False, shareY=False):
        ...
        for i in range(1, twinxs + 1):
            ax = self.pl.fig.add_subplot(
                twinxs * 100 + 10 + i
            )  # , sharex=self.pl.twinxs[0] if shareX and i>1 else "None", sharey=self.pl.twinxs[0] if shareY and i>1 else "None")
    ...
    def draw(self):
        ...
        if len(types) > 1:
            ...
            for rowi in range(self.samplesModel.rowCount()):
                if rowi in selIndsSamples:
                    ...
                    for j in range(len(data.msScan.mz_list)):
                        ...
                        for itemSF in selectedResults:
                            ...
                            for cn in range(0, b.cn + 1):
                                if (
                                    abs(mz - b.meanMZ - cn * 1.00335484) * 1e6 / mz
                                    < matchppm
                                ):
                                    used = True
                                    x2.append(mz)
                                    ...
                                    x2.append(mz)
                                    y2.append(0)
                                    y2.append(data.msScan.intensity_list[j])
                                    y2.append(0)
                            for cn in range(-50, 0) + range(b.cn + 1, b.cn + 50):
                                if (
                                    ...
                                    x2out.append(mz)
                                    y2out.append(0)
                                    y2out.append(data.msScan.intensity_list[j])
                                    y2out.append(0)
                            for delta in [
                                2.014102 - 1.007825,  # H
                                ...
                                if abs(mz - b.meanMZ - delta) * 1e6 / mz < matchppm:
                                    used = True
                                    x2heteroIsotope.append(mz)
                                    x2heteroIsotope.append(mz)
                                    x2heteroIsotope.append(mz)
                                    y2heteroIsotope.append(0)
                                    y2heteroIsotope.append(
                                        data.msScan.intensity_list[j]
                                    )
                                    y2heteroIsotope.append(0)
                                if (
                                    abs(mz - b.meanMZ - cn * 1.00335484 - delta)
                                    * 1e6
                                    / mz
                                    < matchppm
                                ):
                            ...
                            for tlii in range(self.results.topLevelItemCount()):
                                ...
                                if tli.data.type == "SignalPair":
                                    ...
                                    for cn in [
                                        ...
                                        if (
                                            abs(mz - bl.meanMZ - cn * 1.00335484)
                                            * 1e6
                                            / mz
                                            < matchppm
                                        ):
                                            ...
                                            x1a.append(mz)
                                            y1a.append(0)
                                            y1a.append(data.msScan.intensity_list[j])
                                            y1a.append(0)
                        if not used:
                            x1.append(mz)
                            x1.append(mz)
                            x1.append(mz)
                            y1.append(0)
                            y1.append(data.msScan.intensity_list[j])
                            y1.append(0)
                    ...
                    for itemSF in selectedResults:
                        ...
                        x3.append(b.meanMZ)
                        y3.append(0)
                        y3.append(1.1 * max(y2 if len(y2) > 0 else y1))
                        y3.append(0)
                        x3.append(b.meanMZ + b.cn * 1.00335484)
                        x3.append(b.meanMZ + b.cn * 1.00335484)
                        x3.append(b.meanMZ + b.cn * 1.00335484)
                        y3.append(0)
                        y3.append(1.1 * max(y2 if len(y2) > 0 else y1))
                    ...
                    self.drawPlot(
                        x1,
                        y1,
                        color="whitesmoke" if len(x2) > 0 else "lightgrey",
                        title=data.fileName,
                        twinxsindex=shownSample % 5,
                    )
                    self.drawPlot(
                        x1a,
                        y1a,
                        color="Slategrey" if len(x2) > 0 else "lightgrey",
                        title=data.fileName,
                        twinxsindex=shownSample % 5,
                    )
                    self.drawPlot(
                    ...
                    if zoom and len(x2) > 0:
                        ylim = (
                            max(y2 + y2out + y3) * -0.05,
                            max(y2 + y2out + y3) * 1.05,
                        )
            ...
            self.drawPlot(
                ...
                color="Firebrick",
                linestyle="None",
                title="Error between theoretical ratio for native carbon isotopologs",
                xlab="MZ",
                ylab="Delta observed - theoretical (%)",
                twinxsindex=0,
            )
            self.drawPlot(
                xl,
                ...
                color="Firebrick",
                linestyle="None",
                title="Error between theoretical ratio for 13C-labeled carbon isotopologs",
                xlab="MZ",
                ylab="Delta observed - theoretical (%)",
                twinxsindex=1,
            )
