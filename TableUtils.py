...
class Table:
    def __initEmpty__(self, tableName=None):
        ...
        self.curs.execute(
            self.__updateTableName(
                "CREATE TABLE :table: (__internalID INTEGER PRIMARY KEY)"
            )
        )
        self.curs.execute(
            self.__updateTableName(
                "CREATE TABLE :table:_COMMENTS (__internalID INTEGER PRIMARY KEY, comment TEXT)"
            )
        )
    ...
    def __init__(self, headers=None, rows=None, tableName=None, comments=None):
        ...
        self.curs.execute(
            self.__updateTableName(
                "CREATE TABLE :table: (__internalID INTEGER PRIMARY KEY,  %s)"
                % (", ".join(["%s %s" % h for h in cols]))
            )
        )
        self.curs.execute(
            self.__updateTableName(
        ...
        for rowi, row in enumerate(rows):
            ...
            self.curs.execute(
                self.__updateTableName(
                    "INSERT INTO :table: (__internalID) VALUES (%d)" % rowi
                )
            )
            ...
            for i in range(0, int(floor(len(cols) // 250) + 1) * 250 + 1, 250)[1:]:
                ...
                self.curs.execute(
                    self.__updateTableName(
                        "UPDATE :table: SET %s WHERE __internalID=%d"
                        % (",".join(["%s=?" % t[0] for t in cols[last:i]]), rowi)
                    ),
        ...
        for comment in comments:
            self.curs.execute(
                self.__updateTableName(
                    "INSERT INTO :table:_COMMENTS(comment) VALUES(?)"
                ),
                [comment],
            )
    ...
    def renameColumn(self, oldColName, newColName):
        ...
        self.curs.execute(
            self.__updateTableName(
                "UPDATE :table: SET %s = %s" % (newColName, oldColName)
            )
        )
        self.conn.commit()
        ...
        self.curs.execute(
            self.__updateTableName("ALTER TABLE :table: RENAME TO :table:_old")
        )
        self.curs.execute(
            self.__updateTableName(
                "CREATE TABLE :table: AS SELECT %s FROM :table:_old" % (",".join(uCols))
            )
        )
    ...
    def getColumns(self):
        ...
        for row in self.curs.execute(
            self.__updateTableName("PRAGMA table_info(:table:)")
        ):
    ...
    def applyFunction(
        ...
        numOfCols = 0
        for row in self.curs.execute(
            self.updateQuery("SELECT __internalID FROM :table:")
        ):
            numOfCols += 1
        ...
        started = time.time()
        for row in self.curs.execute(
            self.updateQuery("SELECT __internalID, :allCols: FROM :table: %s" % where)
        ):
            iID = row[0]
            data = {}
            for i in r:
        ...
        if showProgress:
            ...
            print(
                "\rApplying (function: %s) took %.1f minutes                                                                                                                                                              "
                % (funcName, s)
            )
        ...
        for update in updates:
            ...
            if showProgress:
                ...
                if elapsed > printAfter:
                    ...
                    s = (elapsed * (1 - doneP) / doneP) / 60.0
                    print(
                        "\rUpdating.. |%-30s| %.1f%% (approximately %.1f minutes remaining)"
                        % ("*" * int(doneP * 30), doneP * 100, s),
                    )
        if showProgress:
            s = (time.time() - started) / 60.0
            print(
                "\rUpdating took %.1f minutes                                                                                                        "
                % s
            )
        self.conn.commit()
    ...
    def getData(
        self, cols=None, where=None, orderby=None, getResultsAsBunchObjects=False
    ):
        ret = []
        if where is None:
            where = ""
    ...
    def setData(self, cols, vals, where=None):
        ...
        if where is not None:
            ...
            where = ""
        self.curs.execute(
            self.__updateTableName("UPDATE :table: SET %s %s" % (",".join(sets), where))
        )
    ...
    def addColumn(self, col, colType, defaultValue=""):
        ...
        self.curs.execute(
            self.__updateTableName(
                "ALTER TABLE :table: ADD COLUMN %s %s DEFAULT '%s'"
                % (col, colType, defaultValue)
            )
        )
        self.conn.commit()
        self.saved = False
    ...
    def duplicateColumn(self, col, newColName):
        ...
        self.curs.execute(
            self.__updateTableName(
                "ALTER TABLE :table: ADD COLUMN %s %s" % (newColName, oldCol.format)
            )
        )
        self.curs.execute(
            self.__updateTableName("UPDATE :table: SET %s=%s" % (newColName, col))
        )
    ...
    def addRow(self, data):
        ...
        self.curs.execute(
            self.__updateTableName(
                "INSERT INTO :table: (%s) VALUES(%s)" % (cols, values)
            )
        )
        self.conn.commit()
        self.saved = False
    ...
    def getComments(self):
        ret = []
        for row in self.curs.execute(
            self.__updateTableName("SELECT comment FROM :table:_COMMENTS")
        ):
            if len(row) == 1:
                ret.append(row[0])
            else:
...
class TableUtils:
    ...
    def readFile(file, fType="", commentLineStart="#", **args):
        ...
        if (
            ...
            if not ("sheetName" in args):
                args["sheetName"] = ""
            return TableUtilsXLS.readFile(
                file, sheetName=args["sheetName"], commentLineStart=commentLineStart
            )
        elif fType == "csv" or bn.endswith(".csv"):
            if not ("delim" in args):
                args["delim"] = ";"
            return TableUtilsCSV.readFile(
                file, delim=args["delim"], commentLineStart=commentLineStart
            )
        elif (
            fType == "tsv"
            or bn.endswith(".tsv")
    ...
    def saveFile(table, file, fType="", writeComments=True, **args):
        ...
        if (
            ...
            if not ("tableName" in args):
                args["tableName"] = ""
            return TableUtilsSQL.saveFile(
                table, file, tableName=args["tableName"], writeComments=writeComments
            )
        else:
            raise Exception("Unknown file type. Please use xls, tsv, sqlite")
        ...
        with open(file, "w") as fo:
            ...
            for row in table.curs.execute(table.updateQuery(select)):
                ...
                fo.write(
                    delim.join(
                        [
                            str(c).replace(delim, "-DELIM-") if c is not None else ""
                            for c in row
                        ]
                    )
                )
...
class TableUtilsXLS:
    @staticmethod
    def readFile(
        file, sheetName="", tableName=None, useColumns=None, commentLineStart="#"
    ):
        if sheetName == "":
            bn = basename(file)
            rf = bn.rfind(".")
        ...
        while not firstColFound:
            ...
            while remainingCols:
                ...
                try:
                    if (
                        sh.cell("%s%s" % (col, row)).value is None
                        or str(sh.cell("%s%s" % (col, row)).value) == ""
                    ):
                        remainingCols = False
                        continue
                except:
...
class TableUtilsSQL:
    ...
    @staticmethod
    def saveFile(
        table, file, tableName, where="", order="", select="", writeComments=True
    ):
        raise Exception("Not implemented yet", "Not implemented yet")
