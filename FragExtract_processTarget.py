...
class AnnotatedMSMSSpectra:
    ...
    def delAnnotationsForPeakInScan(self, native=False, labelled=False, index=-1):
        if self.hasAnnotationForPeakInScan(
            native=native, labelled=labelled, index=index
        ):
            todel = []
    ...
    def hasAnnotationForPeakInScan(self, native=False, labelled=False, index=-1):
        ...
        c = self.getAnnotationsForPeakInScan(
            native=native, labelled=labelled, index=index
        )
        return (
            len(
                self.getAnnotationsForPeakInScan(
                    native=native, labelled=labelled, index=index
                )
            )
            > 0
    ...
    def newAnnotationForPeakInScan(
        self, nativeIndex=None, labelledIndex=None, anno=None
    ):
        if anno == None:
            anno = Bunch()
    ...
    def getAnnotatedMSScan(self, native=False, labelled=False):
        ...
        for i in range(len(self.nativeMz_list)):
            if self.hasAnnotationForPeakInScan(
                native=native, labelled=labelled, index=i
            ):
                usePeaks.append(i)
        ...
        if native:
            return (
                [self.nativeMz_list[i] for i in usePeaks],
                [self.nativeIntensity_list[i] for i in usePeaks],
                [
                    self.getAnnotationsForPeakInScan(
                        native=native, labelled=labelled, index=i
                    )
        ...
        if labelled:
            return (
                [self.labelledMz_list[i] for i in usePeaks],
                [self.labelledIntensity_list[i] for i in usePeaks],
                [
                    self.getAnnotationsForPeakInScan(
                        native=native, labelled=labelled, index=i
                    )
    ...
    def getMzCorrectedComplementaryMSScanForAnnotatedMSScan(
        self, native=False, labelled=False, charge=1, isotopeOffset=1.00335
    ):
        if not (native or labelled) or (native and labelled):
            return

        mzs, ints, annos = self.getAnnotatedMSScan(
            native=not (native), labelled=not (labelled)
        )

        for i, mz, annotations in zip(range(len(mzs)), mzs, annos):
            mzs[i] = (
                mz
                + (1.0 if labelled else -1.0)
                * annotations[0].Cn
                * isotopeOffset
                / charge
            )
...
class ProcessTarget:
    def __init__(
        self,
        targets,
        chromatogramFileNative,
        chromatogramFileLabeled,
        fullScanEICppm,
        fullScanThreshold,
        minMSMSPeakIntensityScaled,
        scalePrecursorMZ,
        minXn,
        ...
        annotationElements,
        annotationPPM,
        useParentFragmentConsistencyRule,
        massBankPPMError,
        massBankMinRelAbundance,
        massBankMinimalScore,
        massBankHitsToLoad,
        saveAsTSV,
        saveAsPDF,
        exportAsSirius,
    ...
    def postMessageToProgressWrapper(self, mes, val="", forPID=None):
        ...
        if forPID != -1 and self.queue is not None:
            if mes.lower() == "text":
                self.queue.put(
                    Bunch(pid=forPID, mes="text", val="%d: %s" % (forPID, val))
                )
    ...
    def findClosestScanTo(self, times, startTime, minRT, maxRT):
        ...
        for i in range(len(times)):
            if (
                minRT <= times[i] <= maxRT
                and abs(times[i] - startTime) < bestMatchRTDiff
            ):
                bestMatch = i
    ...
    def scaleMSScan(
        ...
        if precursorMZ == -1 or not self.scalePrecursorMZ:
            ...
            maxInt = max(
                [
                    msScan.intensity_list[index]
                    for index in range(len(msScan.intensity_list))
                    if minMZ <= msScan.mz_list[index] <= maxMZ
                ]
            )
        else:
            minInt = minVal
            maxInt = max(
                [
                    msScan.intensity_list[index]
                    for index in range(len(msScan.intensity_list))
                    if precursorMZ * (1.0 - ppm / 1000000.0)
                    <= msScan.mz_list[index]
                    <= precursorMZ * (1.0 + ppm / 1000000.0)
                ]
            )
        ...
        for i in range(len(msScan.intensity_list)):
            msScan.intensity_list[i] = minVal + (
                maxVal - minVal
            ) * msScan.intensity_list[i] * 1.0 / (maxInt - minInt)

        return msScan

    def scaleMSScanBy(self, intensity_list, by=1.0):
        newInts = [i * by for i in intensity_list]
        return newInts

    def removePeaksAboveMZ(self, msScan, maxMZ):
        msScan.intensity_list = [
            msScan.intensity_list[i]
            for i in range(len(msScan.mz_list))
            if msScan.mz_list[i] <= maxMZ
        ]
        msScan.mz_list = [
            msScan.mz_list[i]
            for i in range(len(msScan.mz_list))
    ...
    def selectMSMSScansForProcessing(
        self,
        mzxmlNative,
        mzxmlLabeled,
        Cn,
        ...
        metaboliteCharge,
        fullScanEventNative,
        fullScanEventLabeled,
        nativeMS2ScanEvent,
        labelledMS2ScanEvent,
    ):
        eic, timesFS, scanIDsFS, mzs = mzxmlNative.getEIC(
            nativeMZ, ppm=self.fullScanEICppm, filterLine=fullScanEventNative
        )
        eicL, timesFSL, scanIDsFSL, mzs = mzxmlLabeled.getEIC(
            nativeMZ + self.labellingOffset * Cn / metaboliteCharge,
            ppm=self.fullScanEICppm,
            filterLine=fullScanEventLabeled,
        )

        TIC, timesMS2Native, scanIdsMS2Native = mzxmlNative.getTIC(
            filterLine=nativeMS2ScanEvent, useMS2=True
        )
        # limit eic to predefined time window
        # print(eic)
        for i in range(len(eic)):
            if minRT <= timesFS[i] / 60.0 <= maxRT:
                pass
            else:
                eic[i] = 0
        maxIntFullScanIndexNative = max(zip(range(len(eic)), eic), key=lambda x: x[1])[
            0
        ]
        maxIntMS2NativeIndex = self.findClosestScanTo(
            timesMS2Native,
            timesFS[maxIntFullScanIndexNative],
            minRT * 60.0,
            maxRT * 60.0,
        )

        TICL, timesMS2Labelled, scanIdsMS2Labelled = mzxmlLabeled.getTIC(
            filterLine=labelledMS2ScanEvent, useMS2=True
        )
        # limit eic to predefined time window
        # print(eic)
        for i in range(len(eicL)):
            if minRT <= timesFSL[i] / 60.0 <= maxRT:
                pass
            else:
                eicL[i] = 0

        maxIntFullScanIndexLabeled = max(
            zip(range(len(eicL)), eicL), key=lambda x: x[1]
        )[0]
        maxIntMS2LabelledIndex = self.findClosestScanTo(
            timesMS2Labelled,
            timesFSL[maxIntFullScanIndexLabeled],
            minRT * 60.0,
            maxRT * 60.0,
        )
        ...
        ######################################
        if False:
            plt.plot([t / 60.0 for t in timesFS], eic, color="black")
            plt.plot([t / 60.0 for t in timesFS], [-i for i in eicL], color="black")

            plt.plot([t / 60.0 for t in timesMS2Native], TIC, color="red")
            plt.plot(
                [t / 60.0 for t in timesMS2Labelled], [-i for i in TICL], color="red"
            )

            plt.axvline(
                x=timesFS[maxIntFullScanIndexNative] / 60.0,
                ymin=-10000,
                ymax=10000,
                linewidth=2,
                color="black",
            ...
            try:
                plt.axvline(
                    x=timesMS2Native[maxIntMS2NativeIndex] / 60.0,
                    ymin=-10000,
                    ymax=10000,
                    linewidth=2,
                    color="yellow",
                ...
                plt.axvline(
                    x=timesMS2Labelled[maxIntMS2LabelledIndex] / 60.0,
                    ymin=-10000,
                    ymax=10000,
                    linewidth=2,
                    color="green",
        ...
        return (
            maxIntFullScanIndexNative,
            maxIntFullScanIndexLabeled,
            maxIntMS2NativeIndex,
            maxIntMS2LabelledIndex,
            timesFS[maxIntFullScanIndexNative],
            timesFSL[maxIntFullScanIndexLabeled],
            timesMS2Native[maxIntMS2NativeIndex],
            timesMS2Labelled[maxIntMS2LabelledIndex],
            scanIDsFS[maxIntFullScanIndexNative],
            scanIDsFSL[maxIntFullScanIndexLabeled],
            scanIdsMS2Native[maxIntMS2NativeIndex],
            scanIdsMS2Labelled[maxIntMS2LabelledIndex],
            eic,
            eicL,
            timesFS,
            timesFSL,
    ...
    def removePeaksBelowThreshold(self, msScan, intThreshold):
        usePeaks = []
        for i, intensity in zip(
            range(len(msScan.intensity_list)), msScan.intensity_list
        ):
    ...
    def cleanScanFromIsotopoes(
        self, msScan, maxPPM, metaboliteCharge, isotopeOffset=1.00335, use=[+1, -1]
    ):
        ...
        for i in range(len(msScan.mz_list) - 1):
            for j in range(i + 1, len(msScan.mz_list)):
                ...
                if (
                    abs(bmz - amz - isotopeOffset / metaboliteCharge) * 1000000.0 / amz
                ) <= maxPPM:
        ...
        msScan.mz_list = [
            msScan.mz_list[i] for i in range(len(msScan.mz_list)) if i not in toDel
        ]
        msScan.intensity_list = [
            msScan.intensity_list[i]
            for i in range(len(msScan.intensity_list))
    ...
    def calculateCn(
        ...
        maxRelError,
        maxCn,
        isotopeOffset=1.00335,
    ):
        retMS = AnnotatedMSMSSpectra(
            scanMS2Native=scanMS2Native,
            scanMS2Labelled=scanMS2Labelled,
            charge=metaboliteCharge,
            ionMode=scanMS2Native.polarity,
        )
            ...
            for j in range(len(scanMS2Labelled.mz_list)):
                ...
                if nMz < (lMz + 1):
                    for n in atoms:
                        if n < nMz / 12:
                            ppmDiff = (
                                abs(lMz - nMz - n * isotopeOffset / metaboliteCharge)
                                * 1e6
                                / nMz
                            )
                            if ppmDiff < 200 and _debug:
                                print(
                                    "mz native: {0: >10.3f}, mz labeled: {1: >10.3f}, Cn: {2: >2d}, ppmdiff: {3: >10.3f}, nInt: {4: >10.3f}, lInt: {5: >10.3f}".format(
                                        nMz, lMz, n, ppmDiff, nInt, lInt
                                    ),
                                )
                            ...
                            if ppmDiff < 200 and _debug:
                                print("ppmDiff less than 200")

            if bestMatch.nInd != -1:
                retMS.newAnnotationForPeakInScan(
                    nativeIndex=bestMatch.nInd,
                    labelledIndex=bestMatch.lInd,
                    anno=Bunch(
                        Cn=bestMatch.Cn,
                        NIndex=bestMatch.nInd,
                        LIndex=bestMatch.lInd,
                        ratioError=bestRatio,
                        ratioSimilarity=bestMatch.ratioSimilarity,
    ...
    def calculateOptimalMatch(
        ...
        scanMS2Annotated = self.calculateCn(
            charge,
            scanMS2Native,
            scanMS2Labelled,
            matchingPPM=matchingPPM,
            maxRelError=maxRelError,
            maxCn=maxCn,
            isotopeOffset=isotopeOffset,
        )

        for lUsedPeakIndex in [
            anno.LIndex for anno in scanMS2Annotated.peakAnnotations
        ]:
            annosFor = [
                anno
                for anno in scanMS2Annotated.peakAnnotations
                if anno.LIndex == lUsedPeakIndex
            ]
    ...
    def removeNonAnnotatedPeaks(self, scanAnnotated):
        usedNativeIndices = sorted(
            list(set([anno.NIndex for anno in scanAnnotated.peakAnnotations]))
        )
        usedLabelledIndices = sorted(
            list(set([anno.LIndex for anno in scanAnnotated.peakAnnotations]))
        )

        scanAnnotated.nativeMz_list = [
            scanAnnotated.nativeMz_list[i] for i in usedNativeIndices
        ]
        scanAnnotated.nativeIntensity_list = [
            scanAnnotated.nativeIntensity_list[i] for i in usedNativeIndices
        ]
        newMappingsNative = {}
        for i in range(len(scanAnnotated.nativeMz_list)):
            newMappingsNative[usedNativeIndices[i]] = i

        scanAnnotated.labelledMz_list = [
            scanAnnotated.labelledMz_list[i] for i in usedLabelledIndices
        ]
        scanAnnotated.labelledIntensity_list = [
            scanAnnotated.labelledIntensity_list[i] for i in usedLabelledIndices
        ]
        newMappingsLabelled = {}
    ...
    def annotatePeaksWithSumFormulas(
        self,
        adductObj,
        scanAnnotated,
        useAtoms,
        ...
        for annotation in scanAnnotated.peakAnnotations:
                ...
                if self.useTracExtractAnnotation:
                    ar[0] = (
                        ...
                        int(
                            floor(
                                (
                                    scanAnnotated.nativeMz_list[annotation.NIndex]
                                    - adductObj.mzoffset / adductObj.charge
                                )
                                / 12.0
                            )
                        ),
            ...
            if len(sfs) > 0:
                ...
                for sf in sfs:
                    ...
                    massSF = (
                        ft.calcMolWeight(elems) - adductObj.mzoffset / adductObj.charge
                    )
                    dppm = (
                        (scanAnnotated.nativeMz_list[annotation.NIndex] - massSF)
                        * 1000000.0
                        / massSF
                    )

                    annotation.generatedSumFormulas[adductObj.name].append(
                        Bunch(sumFormula=sf, deltaPPM=dppm, neutralLossToParent="")
                    )
        ...
        for annotation in scanAnnotated.peakAnnotations:
            for add, sfs in annotation.generatedSumFormulas.items():
                for sf in sfs:
                    ...
                    elemsNeutralLoss = fT.calcDifferenceBetweenElemDicts(
                        elemsFragment, elemsParent
                    )
                    if len(elemsNeutralLoss) == 0 and fT.flatToString(
                        elemsParent
                    ) == fT.flatToString(elemsFragment):
    ...
    def checkForFragmentParentSumFormulaConsistency(
        self, scanMS2Annotated, parentSumFormula, removeImpossibleAnnotations=True
    ):
        fT = formulaTools()
        ret = []

        annos = [
            scanMS2Annotated.getAnnotationsForPeakInScan(native=True, index=i)[0]
            ...
            for psi in range(len(parentSumFormula)):
                ...
                for anno in annos:
                    for adduct in anno.generatedSumFormulas.keys():
                        for index, annoSF in enumerate(
                            anno.generatedSumFormulas[adduct]
                        ):
                            annoSFElems = fT.parseFormula(annoSF.sumFormula)

                            consistentWithAtLeastOneFragmentSumFormula = (
                                consistentWithAtLeastOneFragmentSumFormula
                                or self.checkIfSubset(annoSFElems, psElems)
                            )
    ...
    def saveResultsToTSV(self, target, curs):
        ...
        annotatedSpectrumLabeled = [
            p
            for p in SQLSelectAsObject(
                curs,
                selectStatement="SELECt mzs, ints, annos AS annos FROM MSSpectra WHERE forTarget=%d AND type='labelled_cleaned'"
                % target.id,
            )
        ][0]
        ...
        if len(annotatedSpectrumNative.mzs) > 0:
            annotatedSpectrumNative.mzs = [
                float(mz) for mz in annotatedSpectrumNative.mzs.split(",")
            ]
        else:
            annotatedSpectrumNative.mzs = []
        ...
        if len(annotatedSpectrumNative.ints) > 0:
            annotatedSpectrumNative.ints = [
                float(i) for i in annotatedSpectrumNative.ints.split(",")
            ]
        else:
            annotatedSpectrumNative.ints = []

        if len(annotatedSpectrumNative.annos) > 0:
            annotatedSpectrumNative.annos = pickle.loads(
                base64.b64decode(annotatedSpectrumNative.annos)
            )
        else:
        ...
        if len(annotatedSpectrumLabeled.mzs) > 0:
            annotatedSpectrumLabeled.mzs = [
                float(mz) for mz in annotatedSpectrumLabeled.mzs.split(",")
            ]
        else:
            annotatedSpectrumLabeled.mzs = []
        ...
        if len(annotatedSpectrumLabeled.ints) > 0:
            annotatedSpectrumLabeled.ints = [
                float(i) for i in annotatedSpectrumLabeled.ints.split(",")
            ]
        else:
            annotatedSpectrumLabeled.ints = []

        if len(annotatedSpectrumLabeled.annos) > 0:
            annotatedSpectrumLabeled.annos = pickle.loads(
                base64.b64decode(annotatedSpectrumLabeled.annos)
            )
        else:
            annotatedSpectrumLabeled.annos = []

        with open(
            target.lcmsmsNativeFile + "." + target.targetName + ".tsv", "w"
        ) as fOut:
            fOut.write(
                "\t".join(
                    [
            ...
            for index in range(len(annotatedSpectrumNative.mzs)):
                ...
                b = Bunch(
                    index=index,
                    mz=annotatedSpectrumNative.mzs[index],
                    relInt=annotatedSpectrumNative.ints[index],
                    l_mz=annotatedSpectrumLabeled.mzs[indexLab],
                    l_relInt=annotatedSpectrumLabeled.ints[indexLab],
                    Cn=annotatedSpectrumNative.annos[index].Cn,
                    sumFormulas=annotatedSpectrumNative.annos[
                        index
                    ].generatedSumFormulas,
                )
                b.dmzppm = (b.l_mz - b.mz - b.Cn * 1.00335484) * 1000000.0 / b.mz
    ...
    def saveResultsToSIRIUSMS(self, target, MS1Scan, curs):
        ...
        with open(
            target.lcmsmsNativeFile + "." + target.targetName + ".ms", "w"
        ) as fOut:
            ...
            for index in range(len(annotatedSpectrumNative.mzs)):
                fOut.write(
                    "%.6f %.4f"
                    % (
                        annotatedSpectrumNative.mzs[index],
                        annotatedSpectrumNative.ints[index],
                    )
                )
                fOut.write("\n")
    ...
    def saveResultsToPDF(self, target, curs):
        pdf = canvas.Canvas(
            self.chromatogramNativeFile + "." + target.targetName + ".pdf",
        ...
        currentHeight = writeKeyValuePair(
            "Target name", target.targetName, pdf, currentHeight
        )
        currentHeight = writeKeyValuePair(
            "LC-HRMS/MS file native",
            self.chromatogramNativeFile[(self.chromatogramNativeFile.rfind("/") + 1) :],
            pdf,
            currentHeight,
        )
        currentHeight = writeKeyValuePair(
            "LC-HRMS/MS file native",
            self.chromatogramLabeledFile[
                (self.chromatogramLabeledFile.rfind("/") + 1) :
            ],
            pdf,
            currentHeight,
        )
        currentHeight = writeKeyValuePair(
            "Sum formula", target.parentSumFormula, pdf, currentHeight
        )
        currentHeight = writeKeyValuePair(
            "Adduct",
            "%s (%.4f)" % (target.adduct, target.adductMZOffset),
            pdf,
            currentHeight,
        )

        pdf.line(50, currentHeight + 20 - 4, 540, currentHeight + 20 - 4)
        currentHeight -= 10

        currentHeight = writeKeyValuePair(
            "Native precursor m/z", "%.4f" % target.precursorMZ, pdf, currentHeight
        )
        currentHeight = writeKeyValuePair("Cn", str(target.Cn), pdf, currentHeight)
        currentHeight = writeKeyValuePair(
            "Charge count", str(target.chargeCount), pdf, currentHeight
        )
        currentHeight = writeKeyValuePair(
            "Retention time [min]",
            "%.2f - %.2f" % (target.startRT, target.stopRT),
            pdf,
            currentHeight,
        )
        currentHeight = writeKeyValuePair(
            "Native MS scan event", target.scanEventMS1M, pdf, currentHeight
        )
        currentHeight = writeKeyValuePair(
            "Labeled MS scan event", target.scanEventMS1Mp, pdf, currentHeight
        )
        currentHeight = writeKeyValuePair(
            "Native MS/MS scan event", target.scanEventMS2Native, pdf, currentHeight
        )
        currentHeight = writeKeyValuePair(
            "Labelled MS/MS scan event", target.scanEventMS2Labeled, pdf, currentHeight
        )
        currentHeight = writeKeyValuePair(
            "Native MS/MS scan time / id",
            "%.2f / %s" % (target.scanRTNativeRaw / 60.0, str(target.scanIDNativeRaw)),
            pdf,
            currentHeight,
        )
        currentHeight = writeKeyValuePair(
            "Labeled MS/MS scan time / id",
            "%.2f / %s"
            % (target.scanRTLabeledRaw / 60.0, str(target.scanIDLabelledRaw)),
            pdf,
            currentHeight,
        )
        ...
        currentHeight = writeKeyValuePair(
            "FragExtract version", str(self.feVersion), pdf, currentHeight
        )
        currentHeight -= 20
        ...
        nativeRawSpectrum = [
            ...
            for p in SQLSelectAsObject(
                curs,
                selectStatement="SELECT mzs, ints FROM MSSpectra WHERE forTarget=%d AND type='native_raw'"
                % target.id,
            )
        ][0]
        if len(nativeRawSpectrum.mzs) > 0:
            nativeRawSpectrum.mzs = [
                float(mz) for mz in nativeRawSpectrum.mzs.split(",")
            ]
        else:
            nativeRawSpectrum.mzs = []
        if len(nativeRawSpectrum.ints) > 0:
            nativeRawSpectrum.ints = [
                float(i) for i in nativeRawSpectrum.ints.split(",")
            ]
        else:
            nativeRawSpectrum.ints = []
        ...
        labelledRawSpectrum = [
            ...
            for p in SQLSelectAsObject(
                curs,
                selectStatement="SELECT mzs, ints FROM MSSpectra WHERE forTarget=%d AND type='labelled_raw'"
                % target.id,
            )
        ][0]
        if len(labelledRawSpectrum.mzs) > 0:
            labelledRawSpectrum.mzs = [
                float(mz) for mz in labelledRawSpectrum.mzs.split(",")
            ]
        else:
            labelledRawSpectrum.mzs = []
        if len(labelledRawSpectrum.ints) > 0:
            labelledRawSpectrum.ints = [
                float(i) for i in labelledRawSpectrum.ints.split(",")
            ]
        else:
            labelledRawSpectrum.ints = []
        ...
        dd.append(
            [
                (
                    min(min(nativeRawSpectrum.mzs), min(labelledRawSpectrum.mzs)),
                    self.minMSMSPeakIntensityScaled,
                ),
                (
                    max(max(nativeRawSpectrum.mzs), max(labelledRawSpectrum.mzs)),
                    self.minMSMSPeakIntensityScaled,
                ),
            ]
        )
        colors.append(Color(100 / 255.0, 100 / 255.0, 100 / 255.0))
        ...
        colors.append(Color(107 / 255.0, 142 / 255.0, 35 / 255.0))
        strokeWidth.append(2.1)
        dd.append(
            [
                (target.precursorMZ + 1.00335 * target.Cn / target.chargeCount, -10),
                (target.precursorMZ + 1.00335 * target.Cn / target.chargeCount, 0),
            ]
        )
        colors.append(Color(107 / 255.0, 142 / 255.0, 35 / 255.0))
        strokeWidth.append(2.1)
        ...
        for mz, it, anno in zip(
            annotatedSpectrum.mzs, annotatedSpectrum.ints, annotatedSpectrum.annos
        ):
            dd.append([(mz, it - 0.5), (mz, it + 0.5)])
            colors.append(Color(178 / 255.0, 34 / 255.0, 34 / 255.0))
            strokeWidth.append(1.2)
            dd.append(
                [
                    (mz + 1.00335 * anno.Cn / target.chargeCount, -(it - 0.5)),
                    (mz + 1.00335 * anno.Cn / target.chargeCount, -(it + 0.5)),
                ]
            )
            colors.append(Color(178 / 255.0, 34 / 255.0, 34 / 255.0))
        ...
        dd.append(
            zip([t / 60.0 for t in target.timesFS], [i / ma for i in target.eicFS])
        )
        colors.append(Color(94 / 255.0, 158 / 255.0, 158 / 255.0))
        strokeWidth.append(0.7)
        ma = max(target.eicLFS)
        ma = ma if ma > 0 else 1
        dd.append(
            zip([t / 60.0 for t in target.timesLFS], [-i / ma for i in target.eicLFS])
        )
        colors.append(Color(94 / 255.0, 158 / 255.0, 158 / 255.0))
        strokeWidth.append(0.7)
        ...
        for index in range(len(annotatedSpectrum.mzs)):
            b = Bunch(
                ...
                ratioDifference=annotatedSpectrum.annos[index].ratioError,
            ...
            if totSumForms == 0:
                ...
                m = (
                    fT.calcMolWeight(
                        fT.parseFormula(list(b.sumFormulas.values())[0][0].sumFormula)
                    )
                    + target.adductObj.mzoffset / target.adductObj.charge
                )
                dl.extend(
                    [
                        str(list(b.sumFormulas.values())[0][0].sumFormula),
                        "%.2f" % ((m - b.mz) * 1000000.0 / b.mz),
                        str(list(b.sumFormulas.keys())[0]),
                ...
                for adduct in b.sumFormulas.keys():
                    if len(b.sumFormulas[adduct]) > 0:
                        for sumForm in b.sumFormulas[adduct]:
                            m = (
                                fT.calcMolWeight(fT.parseFormula(sumForm.sumFormula))
                                + target.adductObj.mzoffset / target.adductObj.charge
                            )
                            dl = [
                                "",
            ...
            for match in target.matches.matchedSubstances:
                ...
                dd.append(
                    [
                        (
                            min(
                                min(nativeRawSpectrum.mzs), min(labelledRawSpectrum.mzs)
                            ),
                            self.minMSMSPeakIntensityScaled,
                        ),
                        (
                            max(
                                max(nativeRawSpectrum.mzs), max(labelledRawSpectrum.mzs)
                            ),
                            self.minMSMSPeakIntensityScaled,
                        ),
                    ]
                ...
                ## parent ions match
                dd.append(
                    [(float(match.exactMass), -110), (float(match.exactMass), -120)]
                )
                colors.append(Color(107 / 255.0, 142 / 255.0, 35 / 255.0))
                strokeWidth.append(2.1)
                ...
                try:
                    ...
                    for i in range(len(mzs)):
                        ...
                        colors.append(Color(40 / 255.0, 40 / 255.0, 40 / 255.0))
                        strokeWidth.append(0.35)
                    ...
                    pdf.drawString(
                        50,
                        currentHeight,
                        "Mass spectra could not be fetched properly from the MassBank servers",
                    )
        ...
        try:
            for index, target in enumerate(self.targets):
                ...
                if index == 0:
                    ...
                    self.postMessageToProgressWrapper("max", 100.0, forPID=target.pID)
                    self.postMessageToProgressWrapper(
                        "text",
                        "Parsing file %s"
                        % (
                            self.chromatogramNativeFile[
                                (self.chromatogramNativeFile.rfind("/") + 1) :
                            ]
                        ),
                        forPID=target.pID,
                    )
            ...
            target.timesFS = ""
            target.timesLFS = ""
            target.scanRTNativeRaw = 0.0
            target.scanRTLabeledRaw = 0.0
            createTableFromBunch(
                "Targets",
                target,
                cursor=curs,
            ...
            for index, target in enumerate(self.targets):
                ...
                if index != 0:
                    ...
                    self.postMessageToProgressWrapper("max", 100.0, forPID=target.pID)
                self.postMessageToProgressWrapper(
                    "value", 40.0 + 60.0 * index / len(self.targets), forPID=target.pID
                )
                self.postMessageToProgressWrapper(
                    "text",
                    "Processing file %s, target %.4f"
                    % (
                        self.chromatogramNativeFile[
                            (self.chromatogramNativeFile.rfind("/") + 1) :
                        ],
                ...
                self._process(
                    chromatogramNative, chromatogramLabeled, target, conn, curs
                )
            ...
            self.printMessage(
                "Error in %s: %s" % (self.chromatogramNativeFile, str(ex))
            )
    ...
    def _process(self, chromatogramNative, chromatogramLabeled, target, conn, curs):
        self.printMessage(
            "Processing file: %s; target: %.4f"
            % (self.chromatogramNativeFile, target.precursorMZ)
        )
        ...
        scanEventsMS2M = sorted(
            list(
                chromatogramNative.getFilterLines(
                    includeMS1=False,
                    includeMS2=True,
        ...
        scanEventsMS2Mp = sorted(
            list(
                chromatogramLabeled.getFilterLines(
                    includeMS1=False,
                    includeMS2=True,
                    includePosPolarity=True,
                    includeNegPolarity=True,
        ...
        t = self.selectMSMSScansForProcessing(
            chromatogramNative,
            chromatogramLabeled,
            target.Cn,
            target.startRT,
        ...
        (
            ...
            maxIntMS2NativeIndex,
            maxIntMS2LabelledIndex,
            maxIntFullScanTimeNative,
            maxIntFullScanTimeLabeled,
            timeMS2Native,
            timeMS2Labelled,
            maxIntFullScanIndexNative,
            maxIntFullScanIndexLabeled,
            maxIntMS2NativeScanID,
            maxIntMS2LabelledScanID,
            eicFSNative,
            eicFSLabeled,
            timesFS,
            timesFSL,
        ...
        startInd = min(
            range(len(timesFS)), key=lambda x: abs(timesFS[x] / 60.0 - target.startRT)
        )
        stopInd = min(
            range(len(timesFS)), key=lambda x: abs(timesFS[x] / 60.0 - target.stopRT)
        )
        target.eicFS = eicFSNative[startInd:stopInd]
        target.timesFS = timesFS[startInd:stopInd]

        startInd = min(
            range(len(timesFSL)), key=lambda x: abs(timesFSL[x] / 60.0 - target.startRT)
        )
        stopInd = min(
            range(len(timesFSL)), key=lambda x: abs(timesFSL[x] / 60.0 - target.stopRT)
        )
        ...
        curs.execute(
            "UPDATE Targets SET eicFS=?, eicLFS=?, timesFS=?, timesLFS=? WHERE id=?",
            (
                base64.b64encode(pickle.dumps(target.eicFS)),
                base64.b64encode(pickle.dumps(target.eicLFS)),
                base64.b64encode(pickle.dumps(target.timesFS)),
                base64.b64encode(pickle.dumps(target.timesLFS)),
                target.id,
            ),
        )

        scanFSNative = deepcopy(
            chromatogramNative.getScanByID(maxIntFullScanIndexNative)
        )
        scanFSLabeled = deepcopy(
            chromatogramLabeled.getScanByID(maxIntFullScanIndexLabeled)
        )
        scanMS2Native = deepcopy(chromatogramNative.getScanByID(maxIntMS2NativeScanID))
        scanMS2Labelled = deepcopy(
            chromatogramLabeled.getScanByID(maxIntMS2LabelledScanID)
        )
        # endregion

        if scanMS2Native.polarity != scanMS2Labelled.polarity:
            logging.error(
                " -> Error: Polarity of native and labelled scan are not the same"
            )
            return
        if scanMS2Native.precursorCharge != scanMS2Labelled.precursorCharge:
            logging.error(
                " -> Error: Charge of native and labelled scan are not the same"
            )
            return
        ...
        try:
            scanMS2Native = self.scaleMSScan(
                scanMS2Native,
                maxMZ=target.precursorMZ + 0.5,
                precursorMZ=target.precursorMZ,
                ppm=self.matchingPPM,
            )
            scanMS2Labelled = self.scaleMSScan(
                scanMS2Labelled,
                maxMZ=target.precursorMZ
                + self.labellingOffset * target.Cn / target.chargeCount
                + 0.5,
                precursorMZ=target.precursorMZ
                + self.labellingOffset * target.Cn / target.chargeCount,
                ppm=self.matchingPPM,
            ...
            print(
                "Scaling to precursor not possible. Scaling will be perfomred to max intensive peak"
            )
        ...
        assert (
            maxIntFullScanTimeNative == scanFSNative.retention_time
            and maxIntFullScanTimeLabeled == scanFSLabeled.retention_time
            and timeMS2Native == scanMS2Native.retention_time
            and timeMS2Labelled == scanMS2Labelled.retention_time
        )
        assert isinstance(scanMS2Native, MS2Scan) and isinstance(
            scanMS2Labelled, MS2Scan
        )
        ...
        writeObjectAsSQLInsert(
            ...
            Bunch(
                forTarget=target.id,
                mzs=",".join([str(mz) for mz in scanMS2Native.mz_list]),
                ints=",".join([str(i) for i in scanMS2Native.intensity_list]),
                scanTime=scanMS2Native.retention_time,
                scanID=str(maxIntMS2NativeScanID),
                type="native_raw",
        ...
        writeObjectAsSQLInsert(
            ...
            Bunch(
                forTarget=target.id,
                mzs=",".join([str(mz) for mz in scanMS2Labelled.mz_list]),
                ints=",".join([str(i) for i in scanMS2Labelled.intensity_list]),
                scanTime=scanMS2Labelled.retention_time,
                scanID=str(maxIntMS2LabelledScanID),
                type="labelled_raw",
        ...
        curs.execute(
            "UPDATE Targets SET scanRTNativeRaw=?, scanRTLabeledRaw=?, scanIDNativeRaw=?, scanIDLabelledRaw=? WHERE id=?",
            (
                scanMS2Native.retention_time,
                scanMS2Labelled.retention_time,
                maxIntMS2NativeScanID,
                maxIntMS2LabelledScanID,
                target.id,
            ),
        ...
        scanMS2Native = self.removePeaksBelowThreshold(
            scanMS2Native, intThreshold=self.minMSMSPeakIntensityScaled
        )
        scanMS2Native = self.cleanScanFromIsotopoes(
            scanMS2Native,
            maxPPM=self.matchingPPM,
            metaboliteCharge=target.chargeCount,
            isotopeOffset=1.00335484,
            use=[+1],
        )

        scanMS2Labelled = self.removePeaksBelowThreshold(
            scanMS2Labelled, intThreshold=self.minMSMSPeakIntensityScaled
        )
        scanMS2Labelled = self.cleanScanFromIsotopoes(
            scanMS2Labelled,
            maxPPM=self.matchingPPM,
            metaboliteCharge=target.chargeCount,
            isotopeOffset=1.00335484,
        ...
        scanMS2Annotated = self.calculateOptimalMatch(
            scanMS2Native,
            scanMS2Labelled,
            self.matchingPPM,
            self.maxRelError,
        ...
        if len(scanMS2Annotated.nativeIntensity_list) > 0:
            scanMS2Annotated.nativeIntensity_list = self.scaleMSScanBy(
                scanMS2Annotated.nativeIntensity_list,
                by=100.0 / max(scanMS2Annotated.nativeIntensity_list),
            )
            scanMS2Annotated.labelledIntensity_list = self.scaleMSScanBy(
                scanMS2Annotated.labelledIntensity_list,
                by=100.0 / max(scanMS2Annotated.labelledIntensity_list),
            )

        # region 9. Processing step: annotate peaks with sum formulas
        self.annotatePeaksWithSumFormulas(
            target.adductObj,
            scanMS2Annotated,
            useAtoms,
            atomsRange,
            fixed,
            ppm=self.annotationPPM,
        )
        # endregion

        # region 10. Processing step: calculate putative sum formulas for parent
        genSumForms = self.calcParentSumFormulas(
            target.precursorMZ,
            target.Cn,
            target.adductObj,
            useAtoms,
        ...
        if target.parentSumFormula == "" or target.parentSumFormula == None:
            ...
            if (
                self.useParentFragmentConsistencyRule
            ):  # check for fragment-parent-sumFormula consistency
                target.parentSumFormula = (
                    self.checkForFragmentParentSumFormulaConsistency(
                        scanMS2Annotated,
                        target.parentSumFormula,
                        removeImpossibleAnnotations=True,
                    )
                )
        else:
            fT = formulaTools()
            ...
            if not alsoGenerated:
                logging.warning(
                    "WARNING: The provided sum formula (%s) for this target was not generated by the seven golden rules"
                    % (fT.flatToString(elemsProvided)),
                    ",".join(genSumForms),
                )
        ...
        target.parentSumFormula = "%d [%s]" % (
            len(target.parentSumFormula),
            ", ".join(target.parentSumFormula),
        )

        sumFormsDisplay = (
            target.parentSumFormula.replace("'", "")
            .replace('"', "")
            .replace("[", "")
            .replace("]", "")
        )
        curs.execute(
            "UPDATE Targets SET parentSumFormula='%s' WHERE id=%d"
            % (sumFormsDisplay, target.id)
        )
        # endregion

        # region 11. Processing step: Calculate neutral losses for MSMS peaks
        if (
            len(target.parentSumFormula) == 1
            and isinstance(target.parentSumFormula, str)
            and target.parentSumFormula != ""
        ):  ## neutral losses will only be calculated for provided or unique sum formulas
            self.calcNeutralLosses(
                scanMS2Annotated, target.parentSumFormula, ppm=self.annotationPPM
            )
        # endregion
        ...
        writeObjectAsSQLInsert(
            ...
            Bunch(
                forTarget=target.id,
                mzs=",".join([str(mz) for mz in scanMS2Annotated.nativeMz_list]),
                ints=",".join([str(i) for i in scanMS2Annotated.nativeIntensity_list]),
                annos=base64.b64encode(
                    pickle.dumps(
                        [
                            scanMS2Annotated.getAnnotationsForPeakInScan(
                                native=True, index=i
                            )[0]
                            for i in range(len(scanMS2Annotated.nativeMz_list))
                        ]
        ...
        writeObjectAsSQLInsert(
            ...
            Bunch(
                forTarget=target.id,
                mzs=",".join([str(mz) for mz in scanMS2Annotated.labelledMz_list]),
                ints=",".join(
                    [str(i) for i in scanMS2Annotated.labelledIntensity_list]
                ),
                annos=base64.b64encode(
                    pickle.dumps(
                        [
                            scanMS2Annotated.getAnnotationsForPeakInScan(
                                labelled=True, index=i
                            )[0]
                            for i in range(len(scanMS2Annotated.nativeMz_list))
                        ]
                    )
        ...
        if False:
            try:
                ...
                matches = self.massbankClient.searchForMSMSSpectra(
                    mzValues=scanMS2Annotated.nativeMz_list,
                    intensityValues=scanMS2Annotated.nativeIntensity_list,
                    ionMode=scanMS2Native.polarity,
                    tolerance=self.massBankPPMError,
                    cutoff=self.massBankMinRelAbundance,
                    maxNumResults=self.massBankHitsToLoad,
                    minimalScore=self.massBankMinimalScore,
                )

                if matches.numResults > 0:
                    for match in matches.matchedSubstances:
                        match.forTarget = target.id
                        mzs, relInts = self.massbankClient.getMSMSSpectraForRecordID(
                            match.id
                        )
        ...
        for i in range(len(scanMS2Annotated.nativeMz_list)):
            ...
            eicN, timesN, scanIdsN, mzs = chromatogramNative.getEIC(
                mz=mz,
                ppm=self.matchingPPM,
                filterLine=target.scanEventMS1M,
                removeSingles=False,
                useMS1=False,
                useMS2=True,
            )
            eicL, timesL, scanIdsL, mzs = chromatogramLabeled.getEIC(
                mz=mzL,
                ppm=self.matchingPPM,
                filterLine=target.scanEventMS1Mp,
                removeSingles=False,
                useMS1=False,
                useMS2=True,
            )

            eicN, timesN, scanIdsN = cropEICs(
                eicN, timesN, scanIdsN, target.startRT * 60, target.stopRT * 60
            )
            eicL, timesL, scanIdsL = cropEICs(
                eicL, timesL, scanIdsL, target.startRT * 60, target.stopRT * 60
            )
