...
class ProcessTarget:
    ...
    def _process(self, chromatogram, target, conn, curs):
        ...
        if scanMS2Native.polarity != scanMS2Labelled.polarity:
            print " -> Error: Polarity of native and labelled scan are not the same"
            return
        if scanMS2Native.precursorCharge != scanMS2Labelled.precursorCharge:
            print " -> Error: Charge of native and labelled scan are not the same"
            return
        ...
        if target.parentSumFormula=="" or target.parentSumFormula==None:
            ...
            if not alsoGenerated:
                print "WARNING: The provided sum formula (%s) for this target was not generated by the seven golden rules"%(fT.flatToString(elemsProvided)), ",".join(genSumForms)
