    ...
    def getPeaksFor(self, times, eic, scales=[11, 66]):
        ...
        if len(ret) > 1:
            for i in range(0, ret.nrow):
                peak=Bunch(peakIndex=int(ret.rx(i+1, 12)[0])-1, peakScale=int(ret.rx(i+1, 11)[0]), peakSNR=ret.rx(i+1, 7)[0],
                           peakArea=ret.rx(i+1, 5)[0], peakLeftFlank=int(ret.rx(i+1, 13)[0])-1, peakRightFlank=int(ret.rx(i+1,14)[0])-1)
                retl.append(peak)
