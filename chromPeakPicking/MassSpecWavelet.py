...
class MassSpecWavelet:
    def __init__(self, scriptLocation=None, scales=None, snrTh=None, minScans=None):
        ...
        if self.scriptLocation == None:
            ...
            self.scriptLocation = (
                get_main_dir() + "/chromPeakPicking/MassSpecWaveletIdentification.r"
            )
    ...
    def getPeaksFor(self, timesi, eici, startIndex=None, endIndex=None):
        ...
        eicRC = (
            str([str(v) for v in eic])
            .replace("[", "")
            .replace("]", "")
            .replace("'", "")
        )
        timesRC = (
            str([str(v) for v in times])
            .replace("[", "")
            .replace("]", "")
            .replace("'", "")
        )
        ...
        if len(ret) > 1:
            ...
            for i in range(0, len(ret), 4):
                ...
                peak = Bunch(
                    peakIndex=int(cur[0]),
                    peakScale=float(cur[1]),
                    peakSNR=float(cur[2]),
                    peakArea=float(cur[3]),
                    peakLeftFlank=float(cur[1]),
                    peakRightFlank=float(cur[1]),
                )
                retl.append(peak)
            ...
            for f in range(0, len(retl)):
                ...
                count = 0
                for i in range(
                    max(0, int(peakCenter - retl[f].peakScale)),
                    min(len(eic) - 1, int(peakCenter + retl[f].peakScale)),
                ):
                    if eic[i] > 0:
                        count = count + 1
            ...
            for i1 in range(len(retl)):
                for i2 in range(len(retl)):
                    p1 = retl[i1]
                    p2 = retl[i2]
                    if p1.peakIndex < p2.peakIndex and (p1.peakIndex + p1.peakScale) > (
                        p2.peakIndex - p2.peakScale
                    ):
                        overlap = (p1.peakIndex + p1.peakScale) - (
                            p2.peakIndex - p2.peakScale
                        )
...
if __name__ == "__main__":
    ...
    if True:
        chromatogram.parse_file(
            "H:/180924_463_12C13CDON_Celegans_short_polar/12C13C_DON_onRemus_Exp27.mzXML"
        )
        mz = 297.1331
        ppm = 5.0
        cn = 15
    ...
    startIndex = getLastTimeBefore(times, refTime=0 * 60.0)
    endIndex = getLastTimeBefore(times, refTime=10000 * 60.0)
    ...
    ax.plot([t / 60.0 for t in times], [-e for e in eicL])
