...
class Peak:
    ...
    def __str__(self):
        return "Center: %d leftFlank: %d rightFlank: %d Area:%.0f" % (
            self.peakIndex,
            self.peakLeftFlank,
            self.peakRightFlank,
            self.peakArea,
...
class GradientPeaks:
    ...
    def expandPeak(self, peak, x, y):
        ...
        while goOn and (peak.peakIndex - peak.peakLeftFlank - 1) > 0:
            rat = (
                y[peak.peakIndex - peak.peakLeftFlank - 1]
                / y[peak.peakIndex - peak.peakLeftFlank]
            )
            delta = (
                y[peak.peakIndex - peak.peakLeftFlank]
                - y[peak.peakIndex - peak.peakLeftFlank - 1]
            ) / (
                x[peak.peakIndex - peak.peakLeftFlank]
                - x[peak.peakIndex - peak.peakLeftFlank - 1]
            )
            ...
            if (
                rat > self.minIncreaseRatio
                and rat < 1.0
                and y[peak.peakIndex - peak.peakLeftFlank - 1] >= self.minIntFlanks
            ):
        ...
        while goOn and (peak.peakIndex + peak.peakRightFlank + 1) < len(x):
            rat = (
                y[peak.peakIndex + peak.peakRightFlank + 1]
                / y[peak.peakIndex + peak.peakRightFlank]
            )
            delta = (
                y[peak.peakIndex + peak.peakRightFlank]
                - y[peak.peakIndex + peak.peakRightFlank + 1]
            ) / (
                x[peak.peakIndex + peak.peakRightFlank + 1]
                - x[peak.peakIndex + peak.peakRightFlank]
            )
            ...
            if (
                rat > self.minIncreaseRatio
                and rat < 1.0
                and y[peak.peakIndex + peak.peakRightFlank + 1] >= self.minIntFlanks
            ):
        ...
        peakWidthValid = (
            self.expTime[0] <= peak.peakRightFlank <= self.expTime[1]
            or self.expTime[0] <= peak.peakLeftFlank <= self.expTime[1]
        )
    ...
    def calcPeakProperties(self, x, y, peak):
        ...
        peak.leftInflectionDelta = (
            y[peak.peakIndex] - y[peak.peakIndex - peak.leftInflection]
        )
        peak.rightInflectionDelta = (
            y[peak.peakIndex] - y[peak.peakIndex + peak.rightInflection]
        )
        ...
        return (
            (peak.leftDelta >= self.minDelta and peak.rightDelta >= self.minDelta)
            and (
                peak.leftInflectionDelta >= self.minInflectionDelta
                and peak.rightInflectionDelta >= self.minInflectionDelta
            )
            and peak.baseLineRatio >= self.minBaseLineRatio
        )
    ...
    ######################################################################################
    def getPeaksFor(
        self, times, eic, scales=None, snrTh=0.1, startIndex=None, endIndex=None
    ):
        ...
        for peak in peaks:
            ret.append(
                Bunch(
                    ...
                    peakScale=(peak.peakRightFlank + peak.peakLeftFlank) / 2,
                    peakSNR=100,
                    peakArea=peak.peakArea,
                    peakLeftFlank=peak.peakLeftFlank,
                    peakRightFlank=peak.peakRightFlank,
                )
            )
...
if __name__ == "__main__":
    ...
    if False:
        mzXMLFile = (
            "E:/Cambridge/Jan_2015/wetransfer-cd97f3/QE_PR493_PV_051014-AIF-02.mzXML"
        )
    ...
    sw.stop()
    durationMessages.append(
        "Importing the file took %.1f seconds (1 time)" % sw.getDurationInSeconds()
    )
    ...
    for i in range(100):
        ...
        eicL_raw, times, scanIds, mzs = t.getEIC(
            mz + cn * dmz / z, ppm, filterLine=filterLine
        )
    sw.stop()
    durationMessages.append(
        "Calculating the EICs took %.1f seconds (100 times)" % sw.getDurationInSeconds()
    )
    ...
    for i in range(1000):
        eicN = smoothDataSeries(
            times, eicN_raw, windowLen=9, window=smoothWin, polynom=3
        )
        eicL = smoothDataSeries(
            times, eicL_raw, windowLen=9, window=smoothWin, polynom=3
        )
    sw.stop()
    durationMessages.append(
        "Smoothing the EICs took %.1f seconds (1000 times)" % sw.getDurationInSeconds()
    )
    ...
    sw.stop()
    durationMessages.append(
        "Detecting chromatographic peaks with GradientDescend took %.1f seconds (1000 times)"
        % sw.getDurationInSeconds()
    )
    ...
    "GradientPeaks"
    for peak in peaks:
        peak.minRT = timesMin[peak.peakIndex]
    for peak in peaksL:
        peak.minRT = timesMin[peak.peakIndex]
    printObjectsAsTable(
        peaks,
    ...
    for peak in peaks:
        ...
        plt.plot(
            [
                timesMin[lind],
                timesMin[peak.peakIndex - peak.leftInflection],
                timesMin[peak.peakIndex],
                timesMin[peak.peakIndex + peak.rightInflection],
                timesMin[rind],
            ],
            [
                eic[lind],
                eic[peak.peakIndex - peak.leftInflection],
                eic[peak.peakIndex],
                eic[peak.peakIndex + peak.rightInflection],
                eic[rind],
            ],
            "yD",
        )
        plt.fill_between(
            timesMin[lind : (rind + 1)],
            0,
            eic[lind : (rind + 1)],
            color="red",
            alpha=0.25,
    ...
    for peak in peaksL:
        ...
        plt.plot(
            ...
            [
                eicL[lind],
                eicL[peak.peakIndex - peak.leftInflection],
                eicL[peak.peakIndex],
                eicL[peak.peakIndex + peak.rightInflection],
                eicL[rind],
            ],
            "yD",
        )
    ...
    sw.stop()
    durationMessages.append(
        "Detecting chromatographic peaks with MassSpecWavelet took %.1f seconds (10 times)"
        % sw.getDurationInSeconds()
    )
    ...
    printObjectsAsTable(
        peaks, ["peakAtTime", "peakLeftFlank", "peakIndex", "peakRightFlank"]
    )
    printObjectsAsTable(
        peaksL, ["peakAtTime", "peakLeftFlank", "peakIndex", "peakRightFlank"]
    )
    for peak in peaks:
