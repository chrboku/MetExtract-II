...
class HCA_generic:
    ...
    def plotTree(self, tree, intend="", newIntend="   "):
        if isinstance(tree, HCALeaf):
            ...
            self.plotTree(
                tree.getLeftKid(), intend=intend + newIntend, newIntend=newIntend
            )
            print(
                intend,
                "Composite + ID:",
            ...
            self.plotTree(
                tree.getRightKid(), intend=intend + newIntend, newIntend=newIntend
            )
    ...
    def splitTreeWithCallback(self, tree, callBackFunction, recursive=True):
        if callBackFunction(tree, hca=self):
            if isinstance(tree, HCAComposite):
                ...
                if recursive:
                    subClusts.extend(
                        self.splitTreeWithCallback(
                            tree=tree.getLeftKid(), callBackFunction=callBackFunction
                        )
                    )
                    subClusts.extend(
                        self.splitTreeWithCallback(
                            tree=tree.getRightKid(), callBackFunction=callBackFunction
                        )
                    )
                else:
    ...
    def getObjsOrderInTree(self, tree):
        if isinstance(tree, HCALeaf):
            ...
            return self.getObjsOrderInTree(tree.getLeftKid()) + self.getObjsOrderInTree(
                tree.getRightKid()
            )
