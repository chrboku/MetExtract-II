...
class formulaTools:
    def __init__(self, elemDetails=None):
        if elemDetails is None:
            ...
            self.elemDetails["D"] = [
                ...
                2.014102,
                0.001,
            ]  ## ATTENTION May be wrong. Just used for Cambridge
            self.elemDetails["2H"] = [
                "Hydrogen",
                "H",
                2,
    ...
    def _parseStruct(self, formula, pos):
        ...
        if formula[pos] == "(":
            ...
            if (
                (pos + 1) < len(formula)
                and formula[pos + 1].isalpha()
                and formula[pos + 1].islower()
            ):
    ...
    def getElementFor(self, elem):
        if not (self.isIso(elem)):
            ...
            if (
                (pos + 1) < len(elem)
                and elem[pos + 1].isalpha()
                and elem[pos + 1].islower()
            ):
        ...
        for iso in fIso:
            for cIso in fIso[iso]:
                ret = (
                    ret
                    + (
                        self.elemDetails[str(cIso[0] + iso)][3]
                        - self.elemDetails[iso][3]
                    )
                    * cIso[1]
                )
        return ret
    ...
    def getAbundance(self, elems):
        ...
        for iso in fIso:
            for cIso in fIso[iso]:
                ret = ret * self.getIsotopologueRatio(
                    fElems[iso], cIso[1], self.elemDetails[str(cIso[0]) + iso][4]
                )
        return ret
    ...
    def calcMolWeight(self, elems):
        ...
        for elem in elems.keys():
            if not (self.isIso(elem)):
                ...
                mw = (
                    mw
                    + self.elemDetails[iso + curElem][3] * elems[elem]
                    - self.elemDetails[curElem][3] * elems[elem]
                )

        return mw

    # returns putaive isotopes for a given mz difference
    def getPutativeIsotopes(
        self, mzdiff, atMZ, z=1, ppm=5.0, maxIsoCombinations=1, used=[]
    ):
        mzdiff = mzdiff * z
    ...
    def flatToString(
        self, elems, prettyPrintWithHTMLTags=False, subStart="<sub>", subEnd="</sub>"
    ):
        ...
        return "".join(
            [
                (
                    "%s%s%d%s" % (e, subStart, fElems[e], subEnd)
                    if fElems[e] > 1
                    else "%s" % e
                )
                for e in sorted(fElems.keys())
            ]
        )
    ...
    def calcDifferenceBetweenSumFormulas(self, sfFragment, sfParent):
        return self.calcDifferenceBetweenElemDicts(
            self.parseFormula(sfFragment), self.parseFormula(sfParent)
        )
    ...
    for form in formulas:
        ...
        res.append(
            [
                form,
                str(elems),
                "%.5f" % fT.calcMolWeight(elems),
                "%.5f" % (fT.calcMolWeight(elems) + 1.007276),
                "%.5f" % (fT.calcMolWeight(elems) + 18.033823),
                "%.5f" % (fT.calcMolWeight(elems) + 22.989218),
                "%.5f" % (fT.calcMolWeight(elems) + 44.998201),
                "%.5f" % (fT.calcMolWeight(elems) - 1.007276),
                fT.flatToString(elems),
            ]
        )
