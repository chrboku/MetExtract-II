...
def generatePageFor(
    ...
    pdf.drawString(
        20,
        820,
        "Num: %s, OGroup: %s,  MZ: %.5f, LMZ: %.5f, Xn: %d,  RT: %.2f, ScanEvent: %s, Ionisation mode: %s"
        % (
            feature.num,
            feature.ogroup,
            feature.mz,
    ...
    AreaDD.append([((feature.rt - feature.rtBorders) - 0.1, -2.75 * ppm)])
    ...
    AreaLDD.append([((feature.rt - feature.rtBorders) - 0.1, -2.75 * ppm)])
    ...
    for i, expGroup in enumerate(experimentalGroups):
        for file in expGroup.files:
            ...
            try:
                ...
                ## Native EICs
                eic, times, scanIds, mzs = mzXMLs[file].getEIC(
                    mz=feature.mz,
                    ppm=ppm,
                    ...
                    mval = max(
                        ...
                        max(
                            [
                                ...
                                if (feature.rt - feature.rtBorders)
                                <= (times[j] / 60.0)
                ...
                eicsSeparatedDD.append(
                    [
                        (
                            (times[j] + showGroupRTShift * (i + 3) * 60) / 60.0
                            - feature.rt,
                            eic[j],
                        )
                        for j in range(len(eic))
            ...
            try:
                ## Labeled EICs
                eic, times, scanIds, mzsL = mzXMLs[file].getEIC(
                    mz=feature.lmz,
                    ppm=ppm,
                    filterLine=useFilterLine,
                    removeSingles=False,
            ...
            try:
                ...
                bestScanIndex = max(
                    [
                        (j, ab)
                        for j, ab in enumerate(eic)
                        if (feature.rt - feature.rtBorders / 3)
                        <= (times[j] / 60.0)
                        <= (feature.rt + feature.rtBorders / 3)
                    ],
                    key=lambda x: x[1],
                )[0]

                scan = mzXMLs[file].getIthMS1Scan(
                    index=bestScanIndex, filterLine=useFilterLine
                )
                uInds = [
                    j
                    for j, mz in enumerate(scan.mz_list)
                    if feature.mz - 3 <= mz <= feature.lmz + 3
                ]
                f = []
                for uInd in uInds:
                    ...
                    if (
                        abs(mz - feature.mz) * 1e6 / feature.mz <= 5.0
                        or abs(mz - feature.lmz) * 1e6 / feature.lmz <= 5.0
                    ):
                        scanDD.append([(mz, 0), (mz, ab)])
                        scanCols.append(
                            matplotlib.colors.cnames[expGroup.color.lower()]
                        )
                        scanWidths.append(1.0)
                    elif (
                        abs(mz - feature.mz - 1.00335484) * 1e6 / feature.mz <= 5.0
                        or abs(mz + 1.00335484 - feature.lmz) * 1e6 / feature.lmz <= 5.0
                    ):
                        scanDD.append([(mz, 0), (mz, ab)])
                        scanCols.append(
                            matplotlib.colors.cnames[expGroup.color.lower()]
                        )
                        scanWidths.append(0.5)
                    else:
                        scanBackgroundDD.append([(mz, 0), (mz, ab)])
                        scanBackgroundCols.append("#396FAC")
                        scanBackgroundWidths.append(0.1)
            except Exception as ex:
                print("-- skipping for", file, "(message: %s)" % str(ex))

            ## get chromatogram area
            scans, times, scanIDs = mzXMLs[file].getSpecificArea(
            ...
            for j, scan in enumerate(scans):
                ...
                for k in range(len(scan)):
                    ...
                    AreaDD.append([(rt / 60.0, (mz - feature.mz) * 1e6 / feature.mz)])
            ...
            for j, scan in enumerate(scans):
                ...
                for k in range(len(scan)):
                    ...
                    AreaLDD.append(
                        [(rt / 60.0, (mz - feature.lmz) * 1e6 / feature.lmz)]
                    )
    ...
    if plotLabeld:
        ...
        pdf.drawString(
            240, 490, "Labeled EICs (artificially separated by assigned group)"
        )
    ...
    pdf.drawString(
        3,
        3,
        "Notes: Raw-data is shown, no processing has been performed, axis labels missing (bug in library), order/color of separated EICs corresponds to group table, names for groups in separated EICs cannot be set",
    )
    ...
    for i, expGroup in enumerate(experimentalGroups):
        ...
        style.append(
            ("BACKGROUND", (0, len(data) - 1), (0, len(data) - 1), expGroup.color)
        )
...
def generatePDF(
    ...
    for metabolite in metabolitesToPlot:
        for feature in metabolite.features:
            importFilter.append(
                (feature.mz * (1.0 - ppm * 5 / 1e6), feature.mz * (1.0 + ppm * 5 / 1e6))
            )
            importFilter.append(
                (
                    feature.lmz * (1.0 - ppm * 5 / 1e6),
                    feature.lmz * (1.0 + ppm * 5 / 1e6),
                )
            )
        ...
        for i, expGroup in enumerate(experimentalGroups):
            ...
            for file in expGroup.files:
                ...
                if file not in mzXMLs.keys():
                    mzXML = Chromatogram()
                    mzXML.parse_file(
                        file, intensityCutoff=intensityCutoff
                    )  # , mzFilter=importFilter)
    ...
    for metabolite in metabolitesToPlot:
        print(
            ("\rProcessing.. |%%-%ds| (%%d/%%d)" % (len(metabolitesToPlot)))
            % ("*" * done, done, len(metabolitesToPlot)),
        )
        if not done % 100:
            if pdf is not None:
                pdf.save()
            pdf = canvas.Canvas(
                saveTo.replace(".pdf", "_%d.pdf" % (done // 100)),
        ...
        if drawMetaboliteIntroPage:
            pdf.drawString(
                20,
                820,
                "Name: %s OGroup: %s Number of features: %d"
                % (metabolite.name, metabolite.ogroup, len(metabolite.features)),
            )
            pdf.showPage()
        ...
        for feature in metabolite.features:
            if pw != None:
                pw.setText(
                    " Metabolite: %s      Num: %s, mz: %.5f, rt: %.2f"
                    % (metabolite.ogroup, feature.num, feature.mz, feature.rt)
                )
            ...
            try:
                generatePageFor(
                    ...
                    pdf,
                    ppm=ppm,
                    plotLabeld=plotLabeld,
                    normEICs=normEICs,
                    showGroupRTShift=showGroupRTShift,
                )
            except Exception as ex:
                print(ex.message)
                pass
...
if __name__ == "__main__" and True:
    ...
    if False:
        ...
        experimentalGroups.append(
            Bunch(
                name="NHC dmalQ",
                files=[
                    path
                    + "/NHC_dmalQ/QE2_jdg_62_Hanson_Niehaus_Hanson_Niehaus_26_1AH_Cells_n.mzXML",
                    path
                    + "/NHC_dmalQ/QE2_jdg_62_Hanson_Niehaus_Hanson_Niehaus_28_1BH_Cells_n.mzXML",
                ],
                color="Firebrick",
            )
        )
        ...
        experimentalGroups.append(
            Bunch(
                ...
                files=[
                    path
                    + "/NHC_dmalQDmaa/QE2_jdg_62_Hanson_Niehaus_Hanson_Niehaus_30_2AH_Cells_n.mzXML",
                    path
                    + "/NHC_dmalQDmaa/QE2_jdg_62_Hanson_Niehaus_Hanson_Niehaus_32_2BH_Cells_n.mzXML",
                ],
                color="Firebrick",
            )
        )
        ...
        experimentalGroups.append(
            Bunch(
                ...
                files=[
                    path
                    + "/NHC_dmnaT/QE2_jdg_62_Hanson_Niehaus_Hanson_Niehaus_10_B1H_Cells_n.mzXML",
                    path
                    + "/NHC_dmnaT/QE2_jdg_62_Hanson_Niehaus_Hanson_Niehaus_11_B2H_Cells_n.mzXML",
                    path
                    + "/NHC_dmnaT/QE2_jdg_62_Hanson_Niehaus_Hanson_Niehaus_12_B3H_Cells_n.mzXML",
                ],
                color="Firebrick",
            )
        )
        ...
        experimentalGroups.append(
            Bunch(
                ...
                files=[
                    path
                    + "/NHC_wildtype/QE2_jdg_62_Hanson_Niehaus_Hanson_Niehaus_7_A1H_Cells_n.mzXML",
                    path
                    + "/NHC_wildtype/QE2_jdg_62_Hanson_Niehaus_Hanson_Niehaus_8_A2H_Cells_n.mzXML",
                    path
                    + "/NHC_wildtype/QE2_jdg_62_Hanson_Niehaus_Hanson_Niehaus_9_A3H_Cells_n.mzXML",
                ],
                color="Firebrick",
            )
        )
        ...
        experimentalGroups.append(
            Bunch(
                ...
                files=[
                    path
                    + "/NNC_dmalQ/QE2_jdg_62_Hanson_Niehaus_Hanson_Niehaus_25_1A_Cells_n.mzXML",
                    path
                    + "/NNC_dmalQ/QE2_jdg_62_Hanson_Niehaus_Hanson_Niehaus_27_1B_Cells_n.mzXML",
                ],
                color="Slategrey",
            )
        )
        ...
        experimentalGroups.append(
            Bunch(
                ...
                files=[
                    path
                    + "/NNC_dmalQDmaa/QE2_jdg_62_Hanson_Niehaus_Hanson_Niehaus_29_2A_Cells_n.mzXML",
                    path
                    + "/NNC_dmalQDmaa/QE2_jdg_62_Hanson_Niehaus_Hanson_Niehaus_31_2B_Cells_n.mzXML",
                ],
                color="Slategrey",
            )
        )
        ...
        experimentalGroups.append(
            Bunch(
                ...
                files=[
                    path
                    + "/NNC_dmnaT/QE2_jdg_62_Hanson_Niehaus_Hanson_Niehaus_4_B1_Cells_n.mzXML",
                    path
                    + "/NNC_dmnaT/QE2_jdg_62_Hanson_Niehaus_Hanson_Niehaus_5_B2_Cells_n.mzXML",
                    path
                    + "/NNC_dmnaT/QE2_jdg_62_Hanson_Niehaus_Hanson_Niehaus_6_B3_Cells_n.mzXML",
                ],
                color="Slategrey",
            )
        )
        ...
        experimentalGroups.append(
            Bunch(
                ...
                files=[
                    path
                    + "/NNC_wildtype/QE2_jdg_62_Hanson_Niehaus_Hanson_Niehaus_1_A1_Cells_n.mzXML",
                    path
                    + "/NNC_wildtype/QE2_jdg_62_Hanson_Niehaus_Hanson_Niehaus_2_A2_Cells_n.mzXML",
                    path
                    + "/NNC_wildtype/QE2_jdg_62_Hanson_Niehaus_Hanson_Niehaus_3_A3_Cells_n.mzXML",
                ],
                color="Slategrey",
            )
        )
        ...
        experimentalGroups.append(
            Bunch(
                ...
                files=[
                    path
                    + "/NHM_dmalQ/QE2_jdg_62_Hanson_Niehaus_Hanson_Niehaus_34_1AH_Media_n.mzXML",
                    path
                    + "/NHM_dmalQ/QE2_jdg_62_Hanson_Niehaus_Hanson_Niehaus_36_1BH_Media_n.mzXML",
                ],
                color="Olivedrab",
            )
        )
        ...
        experimentalGroups.append(
            Bunch(
                ...
                files=[
                    path
                    + "/NHM_dmalQDmaa/QE2_jdg_62_Hanson_Niehaus_Hanson_Niehaus_38_2AH_Media_n.mzXML",
                    path
                    + "/NHM_dmalQDmaa/QE2_jdg_62_Hanson_Niehaus_Hanson_Niehaus_40_2BH_Media_n.mzXML",
                ],
                color="Olivedrab",
            )
        )
        ...
        experimentalGroups.append(
            Bunch(
                ...
                files=[
                    path
                    + "/NHM_dmnaT/QE2_jdg_62_Hanson_Niehaus_Hanson_Niehaus_22_B1H_Media_n.mzXML",
                    path
                    + "/NHM_dmnaT/QE2_jdg_62_Hanson_Niehaus_Hanson_Niehaus_23_B2H_Media_n.mzXML",
                    path
                    + "/NHM_dmnaT/QE2_jdg_62_Hanson_Niehaus_Hanson_Niehaus_24_B3H_Media_n.mzXML",
                ],
                color="Olivedrab",
            )
        )
        ...
        experimentalGroups.append(
            Bunch(
                ...
                files=[
                    path
                    + "/NHM_wildtype/QE2_jdg_62_Hanson_Niehaus_Hanson_Niehaus_19_A1H_Media_n.mzXML",
                    path
                    + "/NHM_wildtype/QE2_jdg_62_Hanson_Niehaus_Hanson_Niehaus_20_A2H_Media_n.mzXML",
                    path
                    + "/NHM_wildtype/QE2_jdg_62_Hanson_Niehaus_Hanson_Niehaus_21_A3H_Media_n.mzXML",
                ],
                color="Olivedrab",
            )
        )
        ...
        experimentalGroups.append(
            Bunch(
                ...
                files=[
                    path
                    + "/NNM_dmalQ/QE2_jdg_62_Hanson_Niehaus_Hanson_Niehaus_33_1A_Media_n.mzXML",
                    path
                    + "/NNM_dmalQ/QE2_jdg_62_Hanson_Niehaus_Hanson_Niehaus_35_1B_Media_n.mzXML",
                ],
                color="Slategrey",
            )
        )
        ...
        experimentalGroups.append(
            Bunch(
                ...
                files=[
                    path
                    + "/NNM_dmalQDmaa/QE2_jdg_62_Hanson_Niehaus_Hanson_Niehaus_37_2A_Media_n.mzXML",
                    path
                    + "/NNM_dmalQDmaa/QE2_jdg_62_Hanson_Niehaus_Hanson_Niehaus_39_2B_Media_n.mzXML",
                ],
                color="Slategrey",
            )
        )
        ...
        experimentalGroups.append(
            Bunch(
                ...
                files=[
                    path
                    + "/NNM_dmnaT/QE2_jdg_62_Hanson_Niehaus_Hanson_Niehaus_16_B1_Media_n.mzXML",
                    path
                    + "/NNM_dmnaT/QE2_jdg_62_Hanson_Niehaus_Hanson_Niehaus_17_B2_Media_n.mzXML",
                    path
                    + "/NNM_dmnaT/QE2_jdg_62_Hanson_Niehaus_Hanson_Niehaus_18_B3_Media_n.mzXML",
                ],
                color="Slategrey",
            )
        )
        ...
        experimentalGroups.append(
            Bunch(
                ...
                files=[
                    path
                    + "/NNM_wildtype/QE2_jdg_62_Hanson_Niehaus_Hanson_Niehaus_13_A1_Media_n.mzXML",
                    path
                    + "/NNM_wildtype/QE2_jdg_62_Hanson_Niehaus_Hanson_Niehaus_14_A2_Media_n.mzXML",
                    path
                    + "/NNM_wildtype/QE2_jdg_62_Hanson_Niehaus_Hanson_Niehaus_15_A3_Media_n.mzXML",
                ],
                color="Slategrey",
            )
        )
        ...
        fps = TableUtils.readFile(
            "E:/Hanson_MetaboliteRepair_AcetylatedSamples/Experiment3/RawData/results.txt",
            sep="\t",
        )
        ...
        for ogroup in ogroups:
            ...
            for row in fps.getData(
                ...
                where="OGroup=%d" % ogroup,
                getResultsAsBunchObjects=True,
            ):
            ...
            if len(features) > 0:
                metabolitesToPlot.append(
                    Bunch(name=ogroup, ogroup=ogroup, features=features)
                )

        generatePDF(
            experimentalGroups,
            metabolitesToPlot,
            saveTo="E:/Hanson_MetaboliteRepair_AcetylatedSamples/Experiment3/RawData/featurePairs_negMode.pdf",
        )
    ...
    if False:
        ...
        experimentalGroups.append(
            Bunch(
                ...
                files=[
                    path
                    + "/PHC_dmalQ/QE2_jdg_62_Hanson_Niehaus_Hanson_Niehaus_26_1AH_Cells_p.mzXML",
                    path
                    + "/PHC_dmalQ/QE2_jdg_62_Hanson_Niehaus_Hanson_Niehaus_28_1BH_Cells_p.mzXML",
                ],
                color="Firebrick",
            )
        )
        ...
        experimentalGroups.append(
            Bunch(
                ...
                files=[
                    path
                    + "/PHC_dmalQDmaa/QE2_jdg_62_Hanson_Niehaus_Hanson_Niehaus_30_2AH_Cells_p.mzXML",
                    path
                    + "/PHC_dmalQDmaa/QE2_jdg_62_Hanson_Niehaus_Hanson_Niehaus_32_2BH_Cells_p.mzXML",
                ],
                color="Firebrick",
            )
        )
        ...
        experimentalGroups.append(
            Bunch(
                ...
                files=[
                    path
                    + "/PHC_dmnaT/QE2_jdg_62_Hanson_Niehaus_Hanson_Niehaus_10_B1H_Cells_p.mzXML",
                    path
                    + "/PHC_dmnaT/QE2_jdg_62_Hanson_Niehaus_Hanson_Niehaus_11_B2H_Cells_p.mzXML",
                    path
                    + "/PHC_dmnaT/QE2_jdg_62_Hanson_Niehaus_Hanson_Niehaus_12_B3H_Cells_p.mzXML",
                ],
                color="Firebrick",
            )
        )
        ...
        experimentalGroups.append(
            Bunch(
                ...
                files=[
                    path
                    + "/PHC_wildtype/QE2_jdg_62_Hanson_Niehaus_Hanson_Niehaus_7_A1H_Cells_p.mzXML",
                    path
                    + "/PHC_wildtype/QE2_jdg_62_Hanson_Niehaus_Hanson_Niehaus_8_A2H_Cells_p.mzXML",
                    path
                    + "/PHC_wildtype/QE2_jdg_62_Hanson_Niehaus_Hanson_Niehaus_9_A3H_Cells_p.mzXML",
                ],
                color="Firebrick",
            )
        )
        ...
        experimentalGroups.append(
            Bunch(
                ...
                files=[
                    path
                    + "/PNC_dmalQ/QE2_jdg_62_Hanson_Niehaus_Hanson_Niehaus_25_1A_Cells_p.mzXML",
                    path
                    + "/PNC_dmalQ/QE2_jdg_62_Hanson_Niehaus_Hanson_Niehaus_27_1B_Cells_p.mzXML",
                ],
                color="Slategrey",
            )
        )
        ...
        experimentalGroups.append(
            Bunch(
                ...
                files=[
                    path
                    + "/PNC_dmalQDmaa/QE2_jdg_62_Hanson_Niehaus_Hanson_Niehaus_29_2A_Cells_p.mzXML",
                    path
                    + "/PNC_dmalQDmaa/QE2_jdg_62_Hanson_Niehaus_Hanson_Niehaus_31_2B_Cells_p.mzXML",
                ],
                color="Slategrey",
            )
        )
        ...
        experimentalGroups.append(
            Bunch(
                ...
                files=[
                    path
                    + "/PNC_dmnaT/QE2_jdg_62_Hanson_Niehaus_Hanson_Niehaus_4_B1_Cells_p.mzXML",
                    path
                    + "/PNC_dmnaT/QE2_jdg_62_Hanson_Niehaus_Hanson_Niehaus_5_B2_Cells_p.mzXML",
                    path
                    + "/PNC_dmnaT/QE2_jdg_62_Hanson_Niehaus_Hanson_Niehaus_6_B3_Cells_p.mzXML",
                ],
                color="Slategrey",
            )
        )
        ...
        experimentalGroups.append(
            Bunch(
                ...
                files=[
                    path
                    + "/PNC_wildtype/QE2_jdg_62_Hanson_Niehaus_Hanson_Niehaus_1_A1_Cells_p.mzXML",
                    path
                    + "/PNC_wildtype/QE2_jdg_62_Hanson_Niehaus_Hanson_Niehaus_2_A2_Cells_p.mzXML",
                    path
                    + "/PNC_wildtype/QE2_jdg_62_Hanson_Niehaus_Hanson_Niehaus_3_A3_Cells_p.mzXML",
                ],
                color="Slategrey",
            )
        )
        ...
        experimentalGroups.append(
            Bunch(
                ...
                files=[
                    path
                    + "/PHM_dmalQ/QE2_jdg_62_Hanson_Niehaus_Hanson_Niehaus_34_1AH_Media_p.mzXML",
                    path
                    + "/PHM_dmalQ/QE2_jdg_62_Hanson_Niehaus_Hanson_Niehaus_36_1BH_Media_p.mzXML",
                ],
                color="Olivedrab",
            )
        )
        ...
        experimentalGroups.append(
            Bunch(
                ...
                files=[
                    path
                    + "/PHM_dmalQDmaa/QE2_jdg_62_Hanson_Niehaus_Hanson_Niehaus_38_2AH_Media_p.mzXML",
                    path
                    + "/PHM_dmalQDmaa/QE2_jdg_62_Hanson_Niehaus_Hanson_Niehaus_40_2BH_Media_p.mzXML",
                ],
                color="Olivedrab",
            )
        )
        ...
        experimentalGroups.append(
            Bunch(
                ...
                files=[
                    path
                    + "/PHM_dmnaT/QE2_jdg_62_Hanson_Niehaus_Hanson_Niehaus_22_B1H_Media_p.mzXML",
                    path
                    + "/PHM_dmnaT/QE2_jdg_62_Hanson_Niehaus_Hanson_Niehaus_23_B2H_Media_p.mzXML",
                    path
                    + "/PHM_dmnaT/QE2_jdg_62_Hanson_Niehaus_Hanson_Niehaus_24_B3H_Media_p.mzXML",
                ],
                color="Olivedrab",
            )
        )
        ...
        experimentalGroups.append(
            Bunch(
                ...
                files=[
                    path
                    + "/PHM_wildtype/QE2_jdg_62_Hanson_Niehaus_Hanson_Niehaus_19_A1H_Media_p.mzXML",
                    path
                    + "/PHM_wildtype/QE2_jdg_62_Hanson_Niehaus_Hanson_Niehaus_20_A2H_Media_p.mzXML",
                    path
                    + "/PHM_wildtype/QE2_jdg_62_Hanson_Niehaus_Hanson_Niehaus_21_A3H_Media_p.mzXML",
                ],
                color="Olivedrab",
            )
        )
        ...
        experimentalGroups.append(
            Bunch(
                ...
                files=[
                    path
                    + "/PNM_dmalQ/QE2_jdg_62_Hanson_Niehaus_Hanson_Niehaus_33_1A_Media_p.mzXML",
                    path
                    + "/PNM_dmalQ/QE2_jdg_62_Hanson_Niehaus_Hanson_Niehaus_35_1B_Media_p.mzXML",
                ],
                color="Slategrey",
            )
        )
        ...
        experimentalGroups.append(
            Bunch(
                ...
                files=[
                    path
                    + "/PNM_dmalQDmaa/QE2_jdg_62_Hanson_Niehaus_Hanson_Niehaus_37_2A_Media_p.mzXML",
                    path
                    + "/PNM_dmalQDmaa/QE2_jdg_62_Hanson_Niehaus_Hanson_Niehaus_39_2B_Media_p.mzXML",
                ],
                color="Slategrey",
            )
        )
        ...
        experimentalGroups.append(
            Bunch(
                ...
                files=[
                    path
                    + "/PNM_dmnaT/QE2_jdg_62_Hanson_Niehaus_Hanson_Niehaus_16_B1_Media_p.mzXML",
                    path
                    + "/PNM_dmnaT/QE2_jdg_62_Hanson_Niehaus_Hanson_Niehaus_17_B2_Media_p.mzXML",
                    path
                    + "/PNM_dmnaT/QE2_jdg_62_Hanson_Niehaus_Hanson_Niehaus_18_B3_Media_p.mzXML",
                ],
                color="Slategrey",
            )
        )
        ...
        experimentalGroups.append(
            Bunch(
                ...
                files=[
                    path
                    + "/PNM_wildtype/QE2_jdg_62_Hanson_Niehaus_Hanson_Niehaus_13_A1_Media_p.mzXML",
                    path
                    + "/PNM_wildtype/QE2_jdg_62_Hanson_Niehaus_Hanson_Niehaus_14_A2_Media_p.mzXML",
                    path
                    + "/PNM_wildtype/QE2_jdg_62_Hanson_Niehaus_Hanson_Niehaus_15_A3_Media_p.mzXML",
                ],
                color="Slategrey",
            )
        )
    ...
    if False:
        ...
        fps = TableUtils.readFile(
            "E:/180523_433_DON_Warth/DONConjugatesDB.tsv", sep="\t"
        )

        metabolitesToPlot = []
        for row in fps.getData(
            cols=["Num", "Name", "Rt_min", "MonoisotopicMass"],
            getResultsAsBunchObjects=True,
        ):
            border = 0.33
            try:
            ...
            metabolitesToPlot.append(
                Bunch(name=row.Name, ogroup=row.Name, features=features)
            )
    ...
    if False:
        ...
        experimentalGroups.append(
            Bunch(
                name="170508_350_MethylatedCompounds",
                files=[
        ...
        experimentalGroups.append(
            Bunch(
                name="180420_422_MSMS_for_Papers",
                files=[
        ...
        experimentalGroups.append(
            Bunch(
                name="180511_417_Trichoderma_Susanne_Innsbruck_Vorversuch",
                files=[
        ...
        experimentalGroups.append(
            Bunch(
                name="180601_434_Sorbicillinoids",
                files=[
        ...
        experimentalGroups.append(
            Bunch(
                name="180702_440_sorbicillinoids_posneg",
                files=[
    ...
    if False:
        ...
        fps = TableUtils.readFile(
            "D:/180710_443_posneg_OrbitrapTests/Standards.txt", sep="\t"
        )
    ...
    if True:
        ...
        experimentalGroups.append(
            Bunch(
                name="Reference (243,244) experiment",
                files=[
        ...
        fps = TableUtils.readFile(
            "E:/180420_422_MSMS_for_Papers/RawData/EVAL_BiologicalPublication_GMLPHE/MatchMSMSDataToDataMatrix/BiologicalPaper/exampleFiles/matchedFeatures.txt",
            sep="\t",
        )
        ...
        for row in fps.getData(
            ...
            features.append(
                Bunch(
                    ...
                    rtBorders=border,
                    filterLine=[
                        "Q Exactive (MS lvl: 1, pol: +)",
                        "LTQ Orbitrap XL (MS lvl: 1, pol: +)",
                    ],
                    comment="[M+H]+",
                )
            )
