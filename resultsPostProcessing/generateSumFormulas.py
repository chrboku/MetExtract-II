...
def processFile(
    ...
    class formulaSearch:
        ...
        def generateSumFormulaCalcObjects(self, x):
            ...
            mz = (
                ...
                * 1.0
                * (
                    self.ppmCorrectionPosMode
                    if ionMode == "+"
                    else self.ppmCorrectionNegMode
                )
                / 1e6
            )
        ...
        def updateSumFormulaCol(self, x):
                ...
                for adduct in self.adducts:
                    if ionMode == adduct[2] and charge == adduct[3]:
                        ...
                        for e in ret:
                            ...
                            ent = (
                                "%s (Ion: %s, MassErrorPPM: %.5f, MassErrorMass: %.5f)"
                                % (
                                    e,
                                    "[M" + adduct[0] + "]",
                                    (m - mT) * 1e6 / m,
            ...
            if len(dbe[smCol + "_CHONP"]) > 0:
                if len(dbe[smCol + "_CHONP"]) > 250:
                    x[smCol + "_CHONP"] = "Too many hits %d" % (
                        len(dbe[smCol + "_CHONP"])
                    )
                else:
            ...
            if len(dbe[smCol + "_CHONS"]) > 0:
                if len(dbe[smCol + "_CHONS"]) > 250:
                    x[smCol + "_CHONS"] = "Too many hits %d" % (
                        len(dbe[smCol + "_CHONS"])
                    )
                else:
            ...
            if len(dbe[smCol + "_CHOPS"]) > 0:
                if len(dbe[smCol + "_CHOPS"]) > 250:
                    x[smCol + "_CHOPS"] = "Too many hits %d" % (
                        len(dbe[smCol + "_CHOPS"])
                    )
                else:
            ...
            if len(dbe[smCol + "_CHONPS"]) > 0:
                if len(dbe[smCol + "_CHONPS"]) > 250:
                    x[smCol + "_CHONPS"] = "Too many hits %d" % (
                        len(dbe[smCol + "_CHONPS"])
                    )
                else:
    ...
    table.applyFunction(
        x.updateSumFormulaCol, showProgress=True, pwMaxSet=pwMaxSet, pwValSet=pwValSet
    )
