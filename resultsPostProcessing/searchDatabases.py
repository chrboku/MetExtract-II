...
class DBSearch:
            ...
            for rowi, row in enumerate(tsvin):
                ...
                if rowi == 0:
                    ...
                    try:
                        ...
                        rt_min = (
                            float(row[headers["Rt_min"]])
                            if row[headers["Rt_min"]] != ""
                            else None
                        )
                        mz = (
                            float(row[headers["MZ"]])
                            if row[headers["MZ"]] != ""
                            else None
                        )
                        polarity = (
                            row[headers["IonisationMode"]]
                            .strip()
                            .replace('"', "DOURBLEPRIME")
                            .replace("'", "PRIME")
                            .replace("\t", "TAB")
                        ...
                        if sumFormula != "":
                            try:
                                ...
                                logging.error(
                                    "DB import error (%s, row: %d): The sumformula (%s) of the entry %s '%s' could not be parsed"
                                    % (dbName, rowi, sumFormula, num, name)
                                )
                                notImported += 1
                        ...
                        logging.error(
                            "DB import error: Could not import row %d (%s)"
                            % (rowi, ex.message)
                        )
                        notImported += 1
    ...
    def _findGeneric(self, list, getValue, valueLeft, valueRight):
        ...
        while min < max and (max - min) > 1:
            ...
            if valueLeft <= getValue(list[cur]) <= valueRight:
                ...
                rightBound = cur
                while (rightBound + 1) < len(list) and getValue(
                    list[rightBound + 1]
                ) <= valueRight:
                    rightBound += 1
    ...
    def searchDBForMZ(
        ...
        if checkXN not in ["Don't use", "Exact", "Minimum"] and not checkXN.startswith(
            "PlusMinus_"
        ):
            logging.error(
                "Unknown option '%s' for parameter checkXN, must be either of 'Don't use', 'Exact', 'Minimum' or 'PlusMinux_X' where X is a positive integer"
                % (checkXN)
            )
            raise Exception(
                "Unknown option '%s' for parameter checkXN, must be either of 'Don't use', 'Exact', 'Minimum' or 'PlusMinux_X' where X is a positive integer"
            ...
            if polarity == adduct[2] and charges == adduct[3]:
                ...
                if ph[0] != -1:
                    for entryi in range(ph[0], ph[1] + 1):
                        ...
                        if (
                            rt_min == None
                            or entry.rt_min == None
                            or (abs(rt_min - entry.rt_min) <= rt_error)
                        ):
                            elems = None
                            if entry.sumFormula != "":
                                fT = formulaTools()
                            ...
                            if (
                                ...
                                or (
                                    checkXN == "Exact"
                                    and element in elems.keys()
                                    and elems[element] == Xn
                                )
                                or (
                                    checkXN == "Minimum"
                                    and element in elems.keys()
                                ...
                                or (
                                    checkXN.startswith("PlusMinus_")
                                    and element in elems.keys()
                                    and abs(elems[element] - Xn)
                                    <= int(checkXN[10 : len(checkXN)])
                                )
                            ):
                                entry = deepcopy(entry)
                                entry.hitType = (
                                    "MZ with %s to DB-mass match" % (adduct[0])
                                )
        ...
        if ph[0] != -1:
            for entryi in range(ph[0], ph[1] + 1):
                ...
                print(entry)
                if entry.polarity == polarity and (
                    rt_min == None
                    or entry.rt_min == None
                    or (abs(rt_min - entry.rt_min) <= rt_error)
                ):
        ...
        for adduct in adducts:
            if charges == adduct[3]:
                ...
                if ph[0] != -1:
                    for entryi in range(ph[0], ph[1] + 1):
                        entry = self.dbEntriesMZ[entryi]
                        if entry.polarity == adduct[2] and (
                            rt_min == None
                            or entry.rt_min == None
                            or (abs(rt_min - entry.rt_min) <= rt_error)
                        ):
                            ...
                            if (
                                ...
                                entry = deepcopy(entry)
                                entry.hitType = (
                                    "Mass to DB-MZ match with %s" % (adduct[0])
                                )
                                entry.matchErrorPPM = (mz - entry.mz) * 1e6 / mz
                                entry.matchErrorMass = mz - entry.mz
                                possibleHits.append(entry)
...
if True and __name__ == "__main__":
    ...
    db.addEntriesFromFile(
        "KEGG", "C:/development/MetaboliteDBs/DBsForMetExtractII/KEGG_compounds.tsv"
    )
    db.optimizeDB()
