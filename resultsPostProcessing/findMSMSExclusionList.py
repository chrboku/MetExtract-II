...
for si, s in enumerate(data):
    ...
    if len(curbin) == 0:
        ...
        if abs(mz - lastS[1]) * 1000000.0 / mz <= ppm:
            curbin.append([rt, mz, si])
        else:
            bins.append(curbin)
            curbin = [[rt, mz, si]]
...
for bini, bin in enumerate(bins):
    ...
    if len(bin) >= minWallScans:
        ...
        avmz = sum([s[1] for s in bin]) * 1.0 / len(bin)
...
fileLineArray.append(
    '		<software name="findMSMSExclusionList" version="%s" />' % (version)
)
...
for featurei, feature in enumerate(exclusionList):
    ...
    fileLineArray.append(
        '				<pt x="%f" y="%f" />' % (feature[0], feature[2])
    )
    fileLineArray.append(
        '				<pt x="%f" y="%f" />' % (feature[1], feature[2])
    )
    fileLineArray.append(
        '				<pt x="%f" y="%f" />' % (feature[0], feature[2])
    )
