...
def combineResults(
    ...
    with open(saveToFile, "w") as tsvFile:
            ...
            if run:
                ...
                for expDesc in experimentsOrder:
                    ...
                    for pHitNum, pHitRow in results[expDesc].items():
                        if (
                            abs(getattr(pHitRow, mzCol) - getattr(result, mzCol))
                            * 1e6
                            / getattr(result, mzCol)
                            <= maxPPMError
                            and abs(getattr(pHitRow, rtCol) - getattr(result, rtCol))
                            <= maxRTShift
                            and getattr(pHitRow, chargeCol)
                            == getattr(result, chargeCol)
                            and getattr(pHitRow, ionModeCol)
                            == getattr(result, ionModeCol)
                            and (
                                (not checkXn)
                                or getattr(pHitRow, xnCol) == getattr(result, xnCol)
                            )
                        ):
                ...
                for expDesc in experimentsOrder:
                    if len(possibleHits[expDesc]) == 0:
                        ...
                        rowVals.extend(
                            [
                                getattr(
                                    results[expDesc][possibleHits[expDesc][0]], numCol
                                ),
                                getattr(
                                    results[expDesc][possibleHits[expDesc][0]], mzCol
                                ),
                                getattr(
                                    results[expDesc][possibleHits[expDesc][0]], rtCol
                                ),
                                getattr(
                                    results[expDesc][possibleHits[expDesc][0]], xnCol
                                ),
                                getattr(
                        ...
                        for t in importantCols:
                            rowVals.append(
                                getattr(results[expDesc][possibleHits[expDesc][0]], t)
                            )
                    else:
                        rowVals.extend(
                            [
                                ";".join(
                                    [expDesc + str(t) for t in possibleHits[expDesc]]
                                ),
                                "%f"
                                % (
                                    sum(
                                        [
                                            getattr(results[expDesc][t], mzCol)
                                            for t in possibleHits[expDesc]
                                        ]
                                    )
                                    / len(possibleHits[expDesc])
                                ),
                                ";".join(
                                    [
                                        "%.2f" % getattr(results[expDesc][t], rtCol)
                                        for t in possibleHits[expDesc]
                                    ]
                                ),
                                ";".join(
                                    [
                                        str(getattr(results[expDesc][t], xnCol))
                                        for t in possibleHits[expDesc]
                                    ]
                                ),
                                getattr(
                        ...
                        rowVals.extend(
                            [
                                ";".join(
                                    [
                                        str(getattr(results[expDesc][t], x))
                                        for t in possibleHits[expDesc]
                                    ]
                                )
                                for x in importantCols
                            ]
                        )
                ...
                for expDesc in experimentsOrder:
                    if len(possibleHits[expDesc]) == 0:
                        ...
                        for colName in experimentsData[expDesc].columns:
                            rowVals.append(
                                getattr(
                                    results[expDesc][possibleHits[expDesc][0]], colName
                                )
                            )
                    else:
                        for colName in experimentsData[expDesc].columns:
        ...
        resWriter.writerow(
            ["## Combination parameters %s" % (b.dumpAsJSon().replace('"', "'"))]
        )
        resWriter.writerow(
            [
