...
def preAppendEICs(
    ...
    for j in range(len(eics)):
        ...
        scanTimes[j] = [
            scanTimes[j][i] + scanDuration * pretend for i in range(len(scanTimes[j]))
        ]
...
def undoPreAppendEICs(
    eics, refTimes, pretend=25, scanDuration=1, removeFront=True, removeBack=False
):
    eics = deepcopy(eics)
...
class XICAlignment:
    ...
    def alignXIC(
        ...
        if align:
            ...
            eics, scantimesEnlarged = preAppendEICs(
                eics, scantimes, pretend=pretend, scanDuration=scanDuration
            )
            ...
            for i in range(len(eics)):
                ...
                for j in range(len(peakss[i])):
                    peakTime = (
                        scantimes[i][peakss[i][j].NPeakCenter] + pretend * scanDuration
                    )
            ...
            for eic in eics:
                ...
                if len(eic) < maxLen:
                    eicsR = (
                        eicsR
                        + ","
                        + ",".join((str(0) for p in range(len(eic), maxLen)))
                    )
        ...
        for peaks in peakss:
            if append:
                ...
                append = True
            peakssR = peakssR + ",".join(
                (
                    str(peak.NPeakCenter) if align else str(peak.NPeakCenterMin)
                    for peak in peaks
                )
            )
            if len(peaks) < maxPeaks:
                peakssR = (
                    peakssR
                    + ","
                    + ",".join((str(0) for p in range(len(peaks), maxPeaks)))
                )
        ...
        ret = r(
            ...
            + eicsR
            + "), c("
            + ",".join([str(f) for f in refTimes])
            + "), c("
            + peakssR
            + "), "
            + str(len(peakss))
            + ", align="
            + ("TRUE" if align else "FALSE")
            + ", npoly="
            + str(nPolynom)
            + ", maxGroupDist="
            + str(int(maxTimeDiff))
    ...
    def getAligendXICs(
        self, eics, scantimes, align=True, nPolynom=3, pretend=100, scanDuration=1
    ):
        assert len(eics) == len(scantimes)
        if len(eics) == 0:
            return [[]]

        eics, scantimes = preAppendEICs(
            eics, scantimes, pretend=pretend, scanDuration=scanDuration
        )
        ...
        for eic in eics:
            ...
            if len(eic) < maxLen:
                eicsR = (
                    eicsR + "," + ",".join([str(0) for p in range(len(eic), maxLen)])
                )
        ...
        retl, refTimes = undoPreAppendEICs(
            retl, [u for u in refTimes], pretend=pretend, scanDuration=scanDuration
        )
