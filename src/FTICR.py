...
class FTICRProcessing:
            ...
            if inti >= intensityThreshold:
                for cn in cns:
                    ...
                    if intj >= intensityThreshold:
                        ...
                        if (
                            (
                                (isotopologsCanBeOmitted and ((ipoRat > 0 and abs(ipoRat - getRatio(nativePurity, cn, 1)) < maxEnrDeviation) or ipoRat == 0) and ((jmoRat > 0 and abs(jmoRat - getRatio(labeledPurity, cn, 1)) < maxEnrDeviation) or jmoRat == 0))
                                or (not isotopologsCanBeOmitted and ipoRat > 0 and abs(ipoRat - getRatio(nativePurity, cn, 1)) < maxEnrDeviation and jmoRat > 0 and abs(jmoRat - getRatio(labeledPurity, cn, 1)) < maxEnrDeviation)
                            )
                            and jpoRat < 0.05
                            and imoRat < 0.05
                        ):
    ...
    def processMSScans(
        ...
        self.addToHistory(
            ...
            % (
                "".join(["%s(%d-%d)" % (atoms[i], atomsRange[i][0], atomsRange[i][1]) for i in range(len(atoms))]),
                useSevenGoldenRules,
            )
        )
        ...
        with open(outFile, "w") as fout:
            ...
            fout.write(
                "\t".join(
                    ...
                    + [a[a.rfind("/") + 1 :].replace(".tsv", "") + "_L" for a in filesOrder]
                )
            )
            fout.write("\n")
                ...
                fout.write(
                    "\t".join(
                        [
                            ...
                            "" if len(f.sfs) != 1 else "%.3f" % ((f.meanMZ - f.sfs[0].theoMZ) * 1e6 / f.sfs[0].theoMZ),
