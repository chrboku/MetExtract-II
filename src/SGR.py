...
class SGRGenerator:
    ...
    def secondGoldenRule(self, useAtoms, curAtoms):
        ...
        return (not (bool(valences % 2)) or not (bool(atomsWithOddValences % 2))) and (valences >= (2 * maxVal)) and (valences >= (2 * (sum(curAtoms) - 1)))
    ...
    def perm(
        ...
        for c in range(max(0, atomsRange[i][0]), atomsRange[i][1] + 1):
            ...
            if abs(t - molMass) < massDiff and (not useSevenGoldenRules or (self.secondGoldenRule(useAtoms, curAtoms) or not (useSecondRule))):
                ret.append("".join([str(useAtoms[j]) + str(curAtoms[j] > 1 and curAtoms[j] or "") for j in range(0, len(useAtoms)) if curAtoms[j] != 0]))
