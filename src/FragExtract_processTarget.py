...
class AnnotatedMSMSSpectra:
    ...
    def getAnnotatedMSScan(self, native=False, labelled=False):
        ...
        if native:
            return (
                ...
                [self.getAnnotationsForPeakInScan(native=native, labelled=labelled, index=i) for i in usePeaks],
            )
        if labelled:
            return (
    ...
    def getMzCorrectedComplementaryMSScanForAnnotatedMSScan(self, native=False, labelled=False, charge=1, isotopeOffset=1.00335):
        ...
        for i, mz, annotations in zip(range(len(mzs)), mzs, annos):
            mzs[i] = mz + (1.0 if labelled else -1.0) * annotations[0].Cn * isotopeOffset / charge
...
class ProcessTarget:
    ...
    def scaleMSScan(
        ...
        if precursorMZ == -1 or not self.scalePrecursorMZ:
            ...
            minInt = minVal
            maxInt = max([msScan.intensity_list[index] for index in range(len(msScan.intensity_list)) if precursorMZ * (1.0 - ppm / 1000000.0) <= msScan.mz_list[index] <= precursorMZ * (1.0 + ppm / 1000000.0)])

        maxInt = msScan.precursor_intensity

        for i in range(len(msScan.intensity_list)):
            msScan.intensity_list[i] = minVal + (maxVal - minVal) * msScan.intensity_list[i] * 1.0 / (maxInt - minInt)
    ...
    def selectMSMSScansForProcessing(
        ...
        if False:
            ...
            plt.plot([t / 60.0 for t in timesMS2Labelled], [-i for i in TICL], color="red")
    ...
    def cleanScanFromIsotopoes(self, msScan, maxPPM, metaboliteCharge, isotopeOffset=1.00335, use=[+1, -1]):
        ...
        for i in range(len(msScan.mz_list) - 1):
            for j in range(i + 1, len(msScan.mz_list)):
                ...
                if (abs(bmz - amz - isotopeOffset / metaboliteCharge) * 1000000.0 / amz) <= maxPPM:
                    ...
                    for n in atoms:
                        if n < nMz / 12:
                            ...
                            if ppmDiff < 200 and _debug:
                                print(
                                    "mz native: {0: >10.3f}, mz labeled: {1: >10.3f}, Cn: {2: >2d}, ppmdiff: {3: >10.3f}, nInt: {4: >10.3f}, lInt: {5: >10.3f}".format(nMz, lMz, n, ppmDiff, nInt, lInt),
                                )
        ...
        for annotation in scanAnnotated.peakAnnotations:
            ...
            if uA[0] == "C":
                if self.useTracExtractAnnotation:
                    ar[0] = (
                        annotation.Cn,
                        int(floor((scanAnnotated.nativeMz_list[annotation.NIndex] - adductObj.mzoffset / adductObj.charge) / 12.0)),
                    )
                    del f[f.index("C")]
                else:
                    ar[0] = annotation.Cn
            ...
            sfs = sfg.findFormulas(
                scanAnnotated.nativeMz_list[annotation.NIndex] - adductObj.mzoffset / adductObj.charge,
                ppm=ppm,
                useAtoms=uA,
                atomsRange=ar,
                fixed=f,
            ...
            if len(sfs) > 0:
                ...
                for sf in sfs:
                    ...
                    dppm = (scanAnnotated.nativeMz_list[annotation.NIndex] - massSF) * 1000000.0 / massSF
    ...
    def saveResultsToTSV(self, target, curs):
        annotatedSpectrumNative = [
            ...
            for p in SQLSelectAsObject(
                curs,
                selectStatement="SELECt mzs, ints, annos AS annos FROM MSSpectra WHERE forTarget=%d AND type='native_cleaned'" % target.id,
            )
        ][0]
        annotatedSpectrumLabeled = [
            ...
            for p in SQLSelectAsObject(
                curs,
                selectStatement="SELECt mzs, ints, annos AS annos FROM MSSpectra WHERE forTarget=%d AND type='labelled_cleaned'" % target.id,
            )
        ][0]
        ...
        with open(target.lcmsmsNativeFile + "." + target.targetName + ".tsv", "w") as fOut:
            ...
            for index in range(len(annotatedSpectrumNative.mzs)):
                ...
                b = Bunch(
                    ...
                    sumFormulas=annotatedSpectrumNative.annos[index].generatedSumFormulas,
                )
    ...
    def saveResultsToPDF(self, target, curs):
        ...
        currentHeight = writeKeyValuePair(
            "LC-HRMS/MS file native",
            self.chromatogramLabeledFile[(self.chromatogramLabeledFile.rfind("/") + 1) :],
            pdf,
            currentHeight,
        )
        ...
        currentHeight = writeKeyValuePair(
            ...
            "%.2f / %s" % (target.scanRTLabeledRaw / 60.0, str(target.scanIDLabelledRaw)),
            pdf,
            currentHeight,
        )
        ...
        nativeRawSpectrum = [
            ...
            for p in SQLSelectAsObject(
                curs,
                selectStatement="SELECT mzs, ints FROM MSSpectra WHERE forTarget=%d AND type='native_raw'" % target.id,
            )
        ][0]
        ...
        labelledRawSpectrum = [
            ...
            for p in SQLSelectAsObject(
                curs,
                selectStatement="SELECT mzs, ints FROM MSSpectra WHERE forTarget=%d AND type='labelled_raw'" % target.id,
            )
        ][0]
        ...
        dd.append(zip([t / 60.0 for t in target.timesFS], [i / ma for i in target.eicFS]))
        colors.append(Color(94 / 255.0, 158 / 255.0, 158 / 255.0))
        strokeWidth.append(0.7)
        ma = max(target.eicLFS)
        ma = ma if ma > 0 else 1
        dd.append(zip([t / 60.0 for t in target.timesLFS], [-i / ma for i in target.eicLFS]))
        colors.append(Color(94 / 255.0, 158 / 255.0, 158 / 255.0))
        strokeWidth.append(0.7)
        ...
        try:
            for index, target in enumerate(self.targets):
                ...
                if index == 0:
                    ...
                    self.postMessageToProgressWrapper(
                        "text",
                        "Parsing file %s" % (self.chromatogramNativeFile[(self.chromatogramNativeFile.rfind("/") + 1) :]),
                        forPID=target.pID,
                    )
            ...
            for index, target in enumerate(self.targets):
                ...
                if index != 0:
                    self.postMessageToProgressWrapper("start", forPID=target.pID)
                    self.postMessageToProgressWrapper("max", 100.0, forPID=target.pID)
                self.postMessageToProgressWrapper("value", 40.0 + 60.0 * index / len(self.targets), forPID=target.pID)
                self.postMessageToProgressWrapper(
                    "text",
    ...
    def _process(self, chromatogramNative, chromatogramLabeled, target, conn, curs):
        ...
        startInd = min(range(len(timesFS)), key=lambda x: abs(timesFS[x] / 60.0 - target.startRT))
        stopInd = min(range(len(timesFS)), key=lambda x: abs(timesFS[x] / 60.0 - target.stopRT))
        target.eicFS = eicFSNative[startInd:stopInd]
        target.timesFS = timesFS[startInd:stopInd]

        startInd = min(range(len(timesFSL)), key=lambda x: abs(timesFSL[x] / 60.0 - target.startRT))
        stopInd = min(range(len(timesFSL)), key=lambda x: abs(timesFSL[x] / 60.0 - target.stopRT))
        ...
        try:
            ...
            scanMS2Labelled = self.scaleMSScan(
                scanMS2Labelled,
                maxMZ=target.precursorMZ + self.labellingOffset * target.Cn / target.chargeCount + 0.5,
        ...
        if target.parentSumFormula == "" or target.parentSumFormula == None:
            ...
            if self.useParentFragmentConsistencyRule:  # check for fragment-parent-sumFormula consistency
                target.parentSumFormula = self.checkForFragmentParentSumFormulaConsistency(
                    scanMS2Annotated,
            ...
            if not alsoGenerated:
                logging.warning(
                    "WARNING: The provided sum formula (%s) for this target was not generated by the seven golden rules" % (fT.flatToString(elemsProvided)),
                    ",".join(genSumForms),
                )
