...
def getCorrelationShifted(xic1, xic2, peakStartMin, peakEndMin, rtShiftsMin=None):
    ...
    for rtShiftMini, rtShiftMin in enumerate(rtShiftsMin):
        ...
        xic1_ = interpolateToNewRTs(
            np.copy(xic1), np.arange(np.min(newRTs), np.max(newRTs) + 0.0001, 0.01)
        )
        xic2_ = interpolateToNewRTs(
            xic2_, np.arange(np.min(newRTs), np.max(newRTs) + 0.0001, 0.01)
        )
    ...
    for curPosN, rt in enumerate(newRTs):
        ...
        if rt <= xic[0, 0]:
            ...
            if xic[bestNextRtInd, 0] == rt:
                ...
                nxic[curPosN, 1] = xic[leftInd, 1] + (
                    xic[rightInd, 1] - xic[leftInd, 1]
...
def correlationFor(xic1, xic2, peakStartMin, peakEndMin):
    ...
    if algorithm == "pearson":
        try:
            ...
            print(
                "There was an error calculating the pearson correlation between the two provided XICs. these are"
            )
            print("XIC 1")
            print(xic1)
            print("")
...
class RunIdentification:
    ...
    def __init__(
        ...
        ma, ea = getIsotopeMass(
            configuredTracer.isotopeA if metabolisationExperiment else labellingisotopeA
        )
        mb, eb = getIsotopeMass(
            configuredTracer.isotopeB if metabolisationExperiment else labellingisotopeB
        )
        xOffset = mb - ma if metabolisationExperiment else xOffset

        if (
            (not metabolisationExperiment and len(ea) > 0 and ea == eb)
            or metabolisationExperiment
            or (ea == "Hydrogen" and eb == "Deuterium")
        ):
            pass
        else:
        ...
        for j in a:
            if "-" in j:
                self.xCounts.extend(
                    range(int(j[0 : j.find("-")]), int(j[j.find("-") + 1 : len(j)]) + 1)
                )
            elif ":" in j:
                self.xCounts.extend(
                    range(int(j[0 : j.find(":")]), int(j[j.find(":") + 1 : len(j)]) + 1)
        ...
        if (
            eicSmoothingWindow == None
            or eicSmoothingWindow == ""
            or eicSmoothingWindow == "none"
        ):
            eicSmoothingWindow = "None"
    ...
    def postMessageToProgressWrapper(self, mes, val=""):
        if self.pID != -1 and self.queue is not None:
            if mes.lower() == "text":
                self.queue.put(
                    Bunch(pid=self.pID, mes="text", val="%d: %s" % (self.pID, val))
                )
    ...
    def getMostLikelyHeteroIsotope(self, foundIsotopes):
        ...
        for iso in foundIsotopes:
            ...
            for cn in isoD:
                ...
                if useCn == -1 or (
                    abs(cnRatioTheo - cnRatio) < abs(useCnRatio - useCnRatioTheo)
                ):
                    useCn = cn
                    useCnRatio = cnRatio
    ...
    def writeConfigurationToDB(self, conn, curs):
        ...
        SQLInsert(
            curs, "config", key="ExperimentOperator", value=self.experimentOperator
        )
        SQLInsert(curs, "config", key="ExperimentID", value=self.experimentID)
        SQLInsert(
            curs, "config", key="ExperimentComments", value=self.experimentComments
        )
        ...
        SQLInsert(
            curs, "config", key="intensityThreshold", value=self.intensityThreshold
        )
        ...
        SQLInsert(
            curs, "config", key="eicSmoothingPolynom", value=self.eicSmoothingPolynom
        )
        SQLInsert(
            curs,
            "config",
        ...
        SQLInsert(
            ...
            value="Center +- %d signals (%d total)"
            % (peakAbundanceUseSignalsSides, peakAbundanceUseSignals),
        )
        ...
        SQLInsert(
            curs, "config", key="checkPeaksRatio", value=str(self.checkPeaksRatio)
        )
        ...
        SQLInsert(
            curs, "config", key="calcIsoRatioNative", value=self.calcIsoRatioNative
        )
        SQLInsert(
            curs, "config", key="calcIsoRatioLabelled", value=self.calcIsoRatioLabelled
        )
        SQLInsert(
            curs, "config", key="calcIsoRatioMoiety", value=self.calcIsoRatioMoiety
        )
        ...
        if self.metabolisationExperiment:
            ...
            SQLInsert(
                ...
                deltaMZ=getIsotopeMass(tracer.isotopeB)[0]
    ...
    def writeSettingsToPDF(self, pdf):
        ...
        pdf.drawString(
            ...
            % (
                ...
                " (Low abundance cutoff for isotopologues: %.0f)"
                % self.intensityThresholdIsotopologs
                if self.lowAbundanceIsotopeCutoff
                else "",
            ),
        )
        currentHeight -= 15
        ...
        pdf.drawString(
            240, currentHeight, "%.1f-%.1f min" % (self.startTime, self.stopTime)
        )
        currentHeight -= 15

        pdf.drawString(70, currentHeight, "Atom count")
        pdf.drawString(
            240, currentHeight, "%s" % (",".join(str(x) for x in self.xCountsString))
        )
        currentHeight -= 15
        ...
        pdf.drawString(
            ...
            currentHeight,
            "Native: %d Labelled: %d"
            % (self.isotopicPatternCountLeft, self.isotopicPatternCountRight),
        )
        currentHeight -= 15
        ...
        pdf.drawString(
            240,
            currentHeight,
            "Native: %.1f%% Labelled: %.1f%%"
            % (self.intensityErrorN * 100.0, self.intensityErrorL * 100.0),
        )
        currentHeight -= 35
        ...
        p = Paragraph(
            str(
                ", ".join(
                    [
                        "%s (m/z: %.4f, z:%d%s)"
                        % (ad.name, ad.mzoffset, ad.charge, ad.polarity)
        ...
        p = Paragraph(
            str(
                ", ".join(
                    [
                        "%s (m: %.4f)" % (el, self.elements[el].weight)
        ...
        p = Paragraph(
            "UUID_ext: " + self.processingUUID, style=getSampleStyleSheet()["Normal"]
        )
        w, h = p.wrap(450, 120)
        p.wrapOn(pdf, 450, 120)
        p.drawOn(pdf, 60, currentHeight - h + 10)
    ...
    def writeCurrentTracerToPDF(self, pdf, tracer):
        ...
        pdf.drawString(
            240, currentHeight, "%s, %s" % (tracer.isotopeA, tracer.isotopeB)
        )
        currentHeight -= 15
        ...
        if tracer.checkRatio:
            ...
            pdf.drawString(
                ...
                currentHeight,
                "%.5f (%.2f:%.2f [v/v])"
                % (tracer.monoisotopicRatio, tracer.amountA, tracer.amountB),
            )
            currentHeight -= 15
    ...
    def findSignalPairs(self, curProgress, mzxml, tracer, reportFunction=None):
        ...
        if self.negativeScanEvent != "None":
            ...
            n = matchPartners(
                ...
                intensityErrorL=self.intensityErrorL,
                purityN=tracer.enrichmentA
                if self.metabolisationExperiment > 1
                else self.purityN,
                purityL=tracer.enrichmentB
                if self.metabolisationExperiment > 1
                else self.purityL,
                ...
                for ionMode in ["+", "-"]:
                    xAct = sorted(
                        [
                            mz
                            for mz in mzs
                            if mz.ionMode == ionMode
                            and mz.xCount == xCount
                            and mz.loading == loading
                        ],
                        key=lambda x: x.mz,
                    )
                    ...
                    if len(xAct) > 0:
                        ...
                        for i in range(1, len(xAct)):
                            ppmDiff = (
                                (xAct[i].mz - xAct[i - 1].mz) * 1000000 / xAct[i - 1].mz
                            )
    ...
    def removeImpossibleFeaturePairClusters(self, mzbins):
        ...
        for ionMode in mzbins.keys():
            mzBinsNew[ionMode] = [
                mzbin
                for mzbin in mzbins[ionMode]
                if len(mzbin.getKids()) >= self.minSpectraCount
            ]
        return mzBinsNew
    ...
    def getMeanRatioOfScans(
        self, eicA, eicB, lib, rib, perfWeighted=True, minInt=1000, minRatiosNecessary=3
    ):
        try:
            if perfWeighted:
                ...
                os = [
                    o
                    for o in range(lib, rib)
                    if eicA[o] >= minInt and eicB[o] >= minInt and eicB[o] > 0
                ]
                ...
                if len(ratios) >= minRatiosNecessary:
                    ...
                    deviation = sum(
                        [ratios[o] * weights[o] for o in range(len(ratios))]
                    )
                    return ratio
                else:
                    return -1
                ...
                ratios = [
                    (eicA[o] / eicB[o])
                    for o in range(lib, rib)
                    if eicA[o] >= minInt and eicB[o] >= minInt
                ]
                if len(ratios) >= minRatiosNecessary:
                    return mean(ratios)
                else:
    ...
    def findChromatographicPeaksAndWriteToDB(
        self, mzbins, mzxml, tracerID, reportFunction=None
    ):
        conn = connect(self.file + getDBSuffix(), isolation_level="DEFERRED")
        conn.execute("""PRAGMA synchronous = OFF""")
        conn.execute("""PRAGMA journal_mode = OFF""")
        ...
        for ionMode in ["+", "-"]:
            ...
            if scanEvent != "None":
                for mzbin, i in zip(mzbins[ionMode], range(0, len(mzbins[ionMode]))):
                    ...
                    eic, times, scanIds, mzs = mzxml.getEIC(
                        meanmz, self.chromPeakPPM, filterLine=scanEvent
                    )
                    ...
                    eicL, times, scanIds, mzsL = mzxml.getEIC(
                        meanmzLabelled, self.chromPeakPPM, filterLine=scanEvent
                    )
                    ...
                    peaksN = []
                    try:
                        peaksN = self.CP.getPeaksFor(
                            times, eicSmoothed, startIndex=startIndex, endIndex=endIndex
                        )
                    except Exception as ex:
                    ...
                    eicLfirstisoconjugate, timesL, scanIdsL, mzsLfirstisoconjugate = (
                        mzxml.getEIC(
                            meanmzLabelled + 1.00335484 / loading,
                            self.chromPeakPPM,
                            filterLine=scanEvent,
                    ...
                    for peakN in peaksN:
                        ...
                        for li, peakL in enumerate(peaksL):
                            ...
                            if (
                                abs(peakN.peakIndex - peakL.peakIndex)
                                <= self.peakCenterError
                            ):
                                if closestMatch == -1 or closestOffset > abs(
                                    peakN.peakIndex - peakL.peakIndex
                                ):
                                    closestMatch = li
                                    closestOffset = abs(
                                        peakN.peakIndex - peakL.peakIndex
                        ...
                        if closestMatch != -1:
                            ...
                            def findBestArtificialShift(
                                eicN, eicL, lb, rb, shiftFrom=0, shiftTo=0
                            ):
                                correlations = []
                            ...
                            if co.correlation >= self.minPeakCorr and (
                                (not self.checkPeaksRatio)
                                or self.minPeaksRatio
                                ...
                                libs = int(
                                    max(
                                        peak.NPeakCenter
                                        - floor(peak.NBorderLeft * 0.33),
                                        peak.LPeakCenter
                                        - floor(peak.LBorderLeft * 0.33),
                                    )
                                )
                                ribs = (
                                    int(
                                        min(
                                            peak.NPeakCenter
                                            + floor(peak.NBorderRight * 0.33),
                                            peak.LPeakCenter
                                            + floor(peak.LBorderRight * 0.33),
                                        )
                                    )
                                    + 1
                                )
                    ...
                    if len(curChromPeaks) > 0:
                        ...
                        for cs in range(len(eic)):
                            if (
                                ...
                                or eicL[cs] > 0
                                or eicfirstiso[cs] > 0
                                or eicLfirstiso[cs] > 0
                                or eicLfirstisoconjugate[cs] > 0
                            ):
                                keepScans.add(cs)
                        keepScans.add(0)
                        ...
                        SQLInsert(
                            ...
                            xicLfirstisoconjugate=";".join(
                                ["%f" % i for i in eicLfirstisoconjugate]
                            ),
                            ...
                            mzsL=";".join(
                                ["%f" % (mzsL[i]) for i in range(0, len(eic))]
                            ),
                            mzsfirstiso=";".join(
                                ["%f" % (mzsfirstiso[i]) for i in range(0, len(eic))]
                            ),
                            mzsLfirstiso=";".join(
                                ["%f" % (mzsLfirstiso[i]) for i in range(0, len(eic))]
                            ),
                            mzsLfirstisoconjugate=";".join(
                                [
                                    "%f" % (mzsLfirstisoconjugate[i])
                                    for i in range(0, len(eic))
                                ]
                            ),
                            times=";".join(
                                ["%f" % (times[i]) for i in range(0, len(times))]
                            ),
                            scanCount=len(eic),
                            allPeaks=base64.b64encode(
                                dumps({"peaksN": peaksN, "peaksL": peaksL})
                            ).decode("utf-8"),
                        )
                        ...
                        for peak in curChromPeaks:
                            ...
                            if self.performCorrectCCount and False:
                                adjcCount = (
                                    adjcCount
                                    + getAtomAdd(self.purityL, peak.xCount)
                                    + getAtomAdd(self.purityN, peak.xCount)
                                )
                            ...
                            SQLInsert(
                                ...
                                heteroAtomsFeaturePairs="",
                                massSpectrumID=0,
                                assignedMZs=base64.b64encode(
                                    dumps(peak.assignedMZs)
                                ).decode("utf-8"),
                                ...
                                comments=base64.b64encode(dumps(peak.comments)).decode(
                                    "utf-8"
                                ),
    ...
    def removeFalsePositiveFeaturePairsAndUpdateDB(
        self, chromPeaks, reportFunction=None
    ):
        conn = connect(self.file + getDBSuffix(), isolation_level="DEFERRED")
        conn.execute("""PRAGMA synchronous = OFF""")
        conn.execute("""PRAGMA journal_mode = OFF""")
        ...
        for a in range(0, len(chromPeaks)):
            ...
            for b in range(0, len(chromPeaks)):
                ...
                if (
                    a != b
                    and peakA.ionMode == peakB.ionMode
                    and (
                        peakA.mz < peakB.mz
                        or abs(peakA.mz - peakB.mz)
                        <= (peakA.mz * 2.5 * self.ppm / 1000000.0)
                    )
                ):
                    # not same feature pair but same ionisation mode

                    if (
                        abs(peakA.NPeakCenter - peakB.NPeakCenter)
                        <= self.peakCenterError
                    ):
                        # same retention time
                        if abs(peakA.mz - peakB.mz) <= (
                            peakA.mz * 2.5 * self.ppm / 1000000.0
                        ):
                            # same mz value
                            if (
                                isinstance(peakA.xCount, float)
                                and (peakB.xCount - peakA.xCount) == 2
                                and peakB.loading == peakA.loading
                                and ((peakB.LPeakArea / peakA.LPeakArea) <= 0.1)
                            ):
                                ...
                                if b not in todel.keys():
                                    todel[b] = []
                                todel[b].append(
                                    "incorrectly matched hetero atoms (most likely O) with "
                                    + str(peakA.mz)
                                    + " "
                                    + str(peakA.xCount)
                                )
                                ...
                                isinstance(peakA.xCount, float)
                                and (peakB.xCount - peakA.xCount) in [1, 2, 3]
                                and peakB.loading == peakA.loading
                            ):
                                ...
                                if a not in todel.keys():
                                    todel[a] = []
                                todel[a].append(
                                    "same mz but reduced number of carbon atoms with "
                                    + str(peakB.mz)
                                    + " "
                                    + str(peakB.xCount)
                                )
                            ...
                            if (
                                isinstance(peakA.xCount, float)
                                and (peakA.xCount * 2) == peakB.xCount
                                ...
                                if a not in todel.keys():
                                    todel[a] = []
                                todel[a].append(
                                    "polymer mismatch with half the number of carbon atoms with "
                                    + str(peakB.mz)
                                    + " "
                                    + str(peakB.xCount)
                                )
                            ...
                            if (
                                ...
                                if a not in todel.keys():
                                    todel[a] = []
                                todel[a].append(
                                    "singly charged mismatch with half the number of carbon atoms with "
                                    + str(peakB.mz)
                                    + " "
                                    + str(peakB.xCount)
                                )
                            ...
                            if (
                                isinstance(peakA.xCount, float)
                                and 0 <= (peakB.xCount - peakA.xCount * 3) <= 2
                                and peakA.loading == 1
                                and peakB.loading == 3
                            ):
                                # a is intermediate pairing of polymer
                                if a not in todel.keys():
                                    todel[a] = []
                                todel[a].append(
                                    "singly charged mismatch with third the number of carbon atoms with "
                                    + str(peakB.mz)
                                    + " "
                                    + str(peakB.xCount)
                                )
                            ...
                            abs(peakB.mz - peakA.mz - 1.00335 / peakA.loading)
                            <= (peakA.mz * 2.5 * self.ppm / 1000000.0)
                            and peakB.xCount == peakA.xCount
                            and peakB.NPeakArea < 2 * peakA.NPeakArea
                        ):
                            ...
                            if b not in todel.keys():
                                todel[b] = []
                            todel[b].append(
                                "increased m/z by one labeling atoms while the number of labeled atoms is identical."
                                + str(peakA.mz)
                            )
                            ...
                            for i in [1, 2, 3]:
                                if (
                                    peakA.loading == peakB.loading
                                    and abs(
                                        peakB.mz
                                        - peakA.mz
                                        - i * self.xOffset / peakA.loading
                                    )
                                    <= peakA.mz * 2.5 * self.ppm / 1000000.0
                                    and isinstance(peakA.xCount, float)
                                    and (peakA.xCount - peakB.xCount) in [1, 2, 3]
                                ):
                                    # b has an mz offset and a reduced number of carbon atoms (by one labelling atom)
                                    if b not in todel.keys():
                                        todel[b] = []
                                    todel[b].append(
                                        "increased mz (by %d) and reduced number of labeling atoms (by %d) with "
                                        % (i, peakA.xCount - peakB.xCount)
                                        + str(peakA.mz)
                                        + " "
                                        + str(peakA.xCount)
                                    )
                                    ...
                                    and abs(
                                        peakB.mz
                                        - peakA.mz
                                        - i * 1.00335 / peakA.loading
                                    )
                                    <= peakA.mz * 2.5 * self.ppm / 1000000.0
                                    and peakA.xCount == peakB.xCount
                                    and peakB.NPeakArea <= peakA.NPeakArea * 0.1
                                ):
                                    # b has an mz offset and a reduced number of carbon atoms (by one labelling atom)
                                    if b not in todel.keys():
                                        todel[b] = []
                                    todel[b].append(
                                        "increased mz (by %d 13C) and same number of labeling atoms with "
                                        % (i)
                                        + str(peakA.mz)
                                        + " "
                                        + str(peakA.xCount)
            ...
            for b in range(0, len(chromPeaks)):
                ...
                if a != b and peakA.ionMode == peakB.ionMode:
                    if (
                        ...
                        # same chrom peak
                        if abs(
                            peakA.mz - peakB.mz - (peakA.lmz - peakA.mz)
                        ) <= peakA.mz * 2 * self.ppm / 1000000.0 and (
                            (
                                isinstance(peakA.xCount, float)
                                and abs(peakA.xCount * 2 - peakB.xCount) <= 1
                            )
                            or (peakA.xCount == peakB.xCount)
                        ):
                            if a not in todel.keys():
                                todel[a] = []
                            todel[a].append(
                                "dimer (1)" + str(peakB.mz) + " " + str(peakB.xCount)
                            )
        delet = []
        for x in todel.keys():
            delet.append(x)
        ...
        for dele in delet:
            ...
            curs.execute(
                "UPDATE allChromPeaks SET comment='%s' WHERE id=%d"
                % (",".join(todel[dele]), cp.id)
            )
    ...
    def annotateFeaturePairs(self, chromPeaks, mzxml, tracer, reportFunction=None):
        ...
        self.postMessageToProgressWrapper(
            "text", "%s: Annotating feature pairs" % tracer.name
        )
        for i in range(0, len(chromPeaks)):
            ...
            for pIso in self.heteroAtoms:
                ...
                if pIsotope.mzOffset < 0:
                    mz = (
                        peak.mz + pIsotope.mzOffset / peak.loading
                    )  # delta m/z is negative, therefore this decreases the search m/z
                else:
                    mz = (
                        peak.lmz + pIsotope.mzOffset / peak.loading
                    )  # delta m/z is positive
                    ...
                    for curScanNum in range(
                        ...
                        if scan is not None:
                            ...
                            if mzBounds != -1:
                                ...
                                isoBounds = scan.getMostIntensePeak(
                                    isoBounds[0], isoBounds[1]
                                )
                                if isoBounds != -1:
            ...
            for i in range(1, self.calcIsoRatioNative + 1):
                ...
                observedRatioMean = weightedMean(
                    [t.ratio for t in isoRatios], [t.refInt for t in isoRatios]
                )
                observedRatioSD = weightedSd(
                    [t.ratio for t in isoRatios], [t.refInt for t in isoRatios]
                )
            ...
            def findMZDifferenceRelativeToXnForMZs(
                mzFrom, mzTo, tmz, fromScan, toScan, mzxml, scanEvent, ppm
            ):
                diffs = []
                for curScanNum in range(fromScan, toScan):
                    scan = mzxml.getIthMS1Scan(curScanNum, scanEvent)
                    if scan is not None:
                        ...
                        if mzBounds != -1:
                            ...
                            refBounds = scan.getMostIntensePeak(
                                refBounds[0], refBounds[1]
                            )
                            ...
                            if refBounds != -1:
                                ...
                                diffs.append(
                                    (abs(isoPeakMZ - peakMZ) - tmz) * 1000000.0 / mzFrom
                                )
        ...
        self.printMessage(
            "%s: Annotating feature pairs done." % tracer.name, type="info"
        )
                ...
                if peakA.mz < peakB.mz and peakA.xCount == peakB.xCount:
                    ...
                    for pIso in self.heteroAtoms:
                        ...
                        if abs(peakB.mz - peakA.mz - pIsotope.mzOffset) <= (
                            max(peakA.mz, peakB.mz) * 2.5 * self.ppm / 1000000.0
                        ):
                            if pIsotope.mzOffset > 0:
                            ...
                            for nHAtoms in range(
                                pIsotope.minCount, pIsotope.maxCount + 1
                            ):
                                if (
                                    abs(ratio - nHAtoms * pIsotope.relativeAbundance)
                                    <= self.hAIntensityError
                                ):
                                    if (
                                        abs(
                                            ratio - nHAtoms * pIsotope.relativeAbundance
                                        )
                                        < bestFitRatio
                                    ):
                                        bestFitRatio = abs(
        ...
        if len(group) <= 40:
            ...
            for pa in group:
                ...
                for pb in group:
                    ...
                    # peakA shall always have the lower mass
                    if peakA.mz < peakB.mz and (
                        len(peakB.heteroAtomsFeaturePairs) == 0
                        or not all(
                            [
                                s.name.startswith("_")
                                for s in peakB.heteroAtomsFeaturePairs
                            ]
                        )
                    ):
                                ...
                                for adB in adducts[peakB.ionMode]:
                                    if (
                                        ...
                                        if (
                                            ...
                                            and peakA.loading == peakB.loading
                                            and abs(
                                                abs(adB.mzoffset - adA.mzoffset)
                                                - 2 * 1.007276
                                            )
                                            <= (
                                                max(peakA.mz, peakB.mz)
                                                * 2.5
                                                * self.ppm
                                                / 1000000.0
                                            )
                                            ...
                                            peakA.loading == adA.charge
                                            and peakB.loading == adB.charge
                                            and abs(
                                                (peakA.mz - adA.mzoffset)
                                                / adA.mCount
                                                * peakA.loading
                                                - (peakB.mz - adB.mzoffset)
                                                / adB.mCount
                                                * peakB.loading
                                            )
                                            <= (
            ...
            def isAdductPairPresent(
                pA, adductAName, adductsA, pB, adductBName, adductsB, checkPartners=True
            ):
                aFound = False
                for add in adductsA:
                    if add.name == adductAName and (
                        not checkPartners
                        or (add.partnerAdd == adductBName and add.toPeak == pB)
                    ):
                        aFound = True
                bFound = False
                for add in adductsB:
                    if add.name == adductBName and (
                        not checkPartners
                        or (add.partnerAdd == adductAName and add.toPeak == pA)
                    ):
                        bFound = True
            ...
            def removeAdductPair(pA, adductAName, adductsA, pB, adductBName, adductsB):
                ...
                for i, add in enumerate(adductsA):
                    if (
                        add.name == adductAName
                        and add.partnerAdd == adductBName
                        and add.toPeak == pB
                    ):
                        aFound.append(i)
                bFound = []
                for i, add in enumerate(adductsB):
                    if (
                        add.name == adductBName
                        and add.partnerAdd == adductAName
                        and add.toPeak == pA
                    ):
                        bFound.append(i)
                        ...
                        if len(self.elements) > 0:
                            ...
                            if not (isinstance(peakA.xCount, str)):
                                ...
                                for pF in pFs:
                                    ...
                                    sf = fT.flatToString(
                                        c, prettyPrintWithHTMLTags=False
                                    )
                                    inSourceFragments[pa][pb].append(
                                        "%.4f-%s" % (peakB.mz, sf)
                                    )
                    ...
                    for pb in group:
                        ...
                        for pc in group:
                            ...
                            if peakA.mz < peakC.mz < peakB.mz:
                                if (
                                    pa in inSourceFragments.keys()
                                    and pb in inSourceFragments[pa].keys()
                                    and pc in inSourceFragments[pa].keys()
                                    and pc in inSourceFragments.keys()
                                    and pb in inSourceFragments[pc].keys()
                                ):
            ...
            for pe in group:
                ...
                for assignedAdduct in peak.adducts:
                    if assignedAdduct in adductsDict.keys():
                        peak.Ms.append(
                            (peak.mz - adductsDict[assignedAdduct].mzoffset)
                            / adductsDict[assignedAdduct].mCount
                            / peak.loading
                        )
    ...
    def groupFeaturePairsUntargetedAndWriteToDB(
        self, chromPeaks, mzxml, tracer, tracerID, reportFunction=None
    ):
        try:
            conn = connect(self.file + getDBSuffix(), isolation_level="DEFERRED")
            conn.execute("""PRAGMA synchronous = OFF""")
                    ...
                    if peakA.mz < peakB.mz:
                        if (
                            ...
                            if False:
                                ...
                                nonShiftCorr, shiftBestCorr, bestShift = (
                                    getCorrelationShifted(
                                        np.transpose(
                                            np.array([peakA.times, peakA.NXICSmoothed])
                                        )[bmin:bmax],
                                        np.transpose(
                                            np.array([peakB.times, peakB.NXICSmoothed])
                                        )[bmin:bmax],
                                        peakA.times[bmin],
                                        peakA.times[bmax],
                            ...
                            meanSilRatioA = weightedMean(
                                silRatiosA, peakA.silRatios.peakNInts
                            )
                            meanSilRatioB = weightedMean(
                                silRatiosB, peakB.silRatios.peakNInts
                            )
                            ...
                            try:
                                silRatiosFold = max(meanSilRatioA, meanSilRatioB) / min(
                                    meanSilRatioA, meanSilRatioB
                                )
                                silRatiosSD = weightedSd(
                                    [
                                        abs(r - meanSilRatioA) / meanSilRatioA
                                        for r in silRatiosA
                                        if min(r, meanSilRatioA) > 0
                                    ]
                                    + [
                                        abs(r - meanSilRatioB) / meanSilRatioB
                                        for r in silRatiosB
                                        if min(r, meanSilRatioB) > 0
                                    ],
                                    peakA.silRatios.peakNInts
                                    + peakB.silRatios.peakNInts,
                                )

                                # check for similar chromatographic peak profile and similar native to labeled ratio
                                if (
                                    pb >= self.minCorrelation
                                    and silRatiosFold <= 1 + max(0.25, 3 * silRatiosSD)
                                ):
                                ...
                                logging.error(
                                    "Error while convoluting two feature pairs, skipping.. (%s)"
                                    % str(e)
                                )
            ...
            self.postMessageToProgressWrapper(
                "text", "%s: Convoluting feature groups" % tracer.name
            )
            ...
            def splitGroupWithHCA(tGroup, correlations):
                ...
                for tG1 in tGroup:
                    ...
                    for tG2 in tGroup:
                        if (
                            tG1 in correlations.keys()
                            and tG2 in correlations[tG1].keys()
                        ):
                ...
                def checkSubCluster(tree, hca, corrThreshold, cutOffMinRatio):
                    if isinstance(tree, HCA_general.HCALeaf):
                        ...
                        aboveThresholdLKid = sum(
                            [corr > corrThreshold for corr in corrsLKid]
                        )
                        aboveThreshold = sum([corr > corrThreshold for corr in corrs])
                        aboveThresholdRKid = sum(
                            [corr > corrThreshold for corr in corrsRKid]
                        )

                        # print(aboveThresholdLKid, aboveThreshold, aboveThresholdRKid)

                    if (
                        (aboveThresholdLKid * 1.0 / len(corrs)) >= cutOffMinRatio
                        and (aboveThreshold * 1.0 / len(corrs)) >= cutOffMinRatio
                        and (aboveThresholdRKid * 1.0 / len(corrs)) >= cutOffMinRatio
                    ):
                        return False
                    else:
                        return True
            ...
            for tGroup in tGroups:
                ...
                self.postMessageToProgressWrapper(
                    "text",
                    "%s: Convoluting feature groups (%d/%d done)"
                    % (tracer.name, done, len(tGroups)),
                )
                ...
                for gi in range(len(groups)):
                    ...
                    if reportFunction is not None:
                        reportFunction(
                            ...
                            "Searching for relationships (%d remaining)"
                            % (len(groups) - gi),
                        )
                    ...
                    self.annotateChromPeaks(
                        group, peaksInGroup
                    )  # store feature pair annotation in the database

                    done = done + 1
                    self.postMessageToProgressWrapper(
                        "text",
                        "%s: Annotating feature groups (%d/%d done)"
                        % (tracer.name, done, len(groups)),
                    )
            ...
            for peak in chromPeaks:
                ...
                curs.execute(
                    ...
                    (
                        ...
                        base64.b64encode(dumps(peak.correlationsToOthers)).decode(
                            "utf-8"
                        ),
                        peak.id,
                    ),
                ...
                curs.execute(
                    ...
                    (
                        base64.b64encode(dumps(peak.heteroAtomsFeaturePairs)).decode(
                            "utf-8"
                        ),
                        peak.id,
                    ),
            ...
            for group in sorted(
                groups,
                key=lambda x: sum([allPeaks[p].NPeakCenterMin / 60.0 for p in x])
                / len(x),
            ):
                SQLInsert(
                    curs,
                ...
                if hasNeg:
                    scan = mzxml.getIthMS1Scan(
                        int(groupMeanElutionIndex), self.negativeScanEvent
                    )
            ...
            conn.commit()
            self.printMessage(
                "%s: Feature grouping done. " % tracer.name
                + str(len(groups))
                + " feature groups",
                type="info",
            )
    ...
    def writeMassSpectraToDB(self, chromPeaks, mzxml, reportFunction=None):
        ...
        for pi in range(len(chromPeaks)):
            ...
            curs.execute(
                "UPDATE chromPeaks SET massSpectrumID=%d WHERE id=%d"
                % (massSpectraWritten[peak.NPeakCenter], peak.id)
            )
    ...
    def writeResultsToFeatureML(self, forFile):
        ...
        exportAsFeatureML.writeFeatureListToFeatureML(
            features, forFile + ".featureML", ppmPM=self.ppm, rtPM=0.25 * 60
        )
    ...
    def writeResultsToTSVFile(self, forFile):
        ...
        for chromPeak in SQLSelectAsObject(
            ...
            setattr(
                chromPeak, "heteroIsotopologues", loads(base64.b64decode(chromPeak.HAs))
            )
        ...
        if self.metabolisationExperiment:
            for tracer in SQLSelectAsObject(
                curs,
                "SELECT t.id AS id, t.name AS name, t.elementCount AS elementCount, t.natural AS isotopeA, t.labelling AS isotopeB, "
                ...
                "t.tracerType AS tracerType FROM tracerConfiguration t",
        ...
        for chromPeak in sorted(chromPeaks, key=lambda x: x.LPeakCenter):
                ...
                "\t".join(
                    [
                        ...
                        for x in [
                            ...
                            chromPeak.mz,
                            chromPeak.lmz,
                            (chromPeak.lmz - chromPeak.mz - chromPeak.tmz)
                            * 1000000.0
                            / chromPeak.mz,
            ...
            for isoRatio in chromPeak.isotopeRatios:
                ...
                csvFile.write(
                    "\t%.5f\t%.5f\t%.5f" % (observedMean, observedSD, theoreticalRatio)
                )
            csvFile.write("\n")
    ...
    def generateFeaturePairOverviewMap(self, chromPeaks, pdf):
        ...
        for gi in set([peak.fGroupID for peak in chromPeaks]):
            ...
            for peak in sorted(
                ...
                key=functools.cmp_to_key(
                    lambda x, y: int(100 * (x.NPeakCenter - y.NPeakCenter))
                    if x.fGroupID == y.fGroupID
                    else int(x.fGroupID - y.fGroupID)
                ),
            ):
        ...
        pdf.drawString(
            20, 810, "Plots with the same symbol/color belong to the same group."
        )
        pdf.drawString(
            20,
            790,
    ...
    def createPDFGroupPages(self, curs, pdf, gPeaks, lGroupID):
        ...
        for gPeakA in gPeaks:
            ...
            oMinDb = int(
                min(oMinDb, int(max(0, gPeak.NPeakCenter - 3 * gPeak.NBorderLeft)))
            )
            oMaxDb = int(
                max(
                    oMaxDb,
        ...
        for row in curs.execute(
            ...
            if correlation >= self.minCorrelation:
                bgColor = colors.Color(
                    ...
                    0.5
                    + 0.4
                    * (correlation - self.minCorrelation)
                    / (1.0 - self.minCorrelation),
                )  # olivedrab
            elif correlation >= 0:
                bgColor = colors.Color(
                    178 / 255.0,
                ...
                bgColor = colors.Color(
                    47 / 255.0, 79 / 255.0, 79 / 255.0, 0.2 - 0.5 * correlation
                )  # firebrick
        ...
        pdf.drawString(
            20,
            810,
            "Feature group %d. %d participating mzs (next page)"
            % (lGroupID, len(gPeaks)),
        )
        pdf.showPage()
    ...
    def plotMSScanForFeaturePair(self, peak, tracer, scanEvent, mzxml, pdf):
        try:
            ...
            for iscan in iscans:
                ...
                for i in range(len(scan.mz_list)):
                    ...
                    for o in [0, 1, peak.xCount - 1, peak.xCount, peak.xCount + 1]:
                        ...
                        if (
                            (dup * (1.0 - self.ppm / 1000000.0))
                            <= cmz
                            <= (dup * (1.0 + self.ppm / 1000000.0))
                        ):
                            dd.append([(cmz, 0), (cmz, cint)])
                            colors.append(
                                Color(178 / 255.0, 34 / 255.0, 34 / 255.0)
                            )  # firebrick
                            strokes.append(1.2)
                    ...
                    if len(peak.heteroIsotopologues) > 0:
                        for pIso in peak.heteroIsotopologues:
                            ...
                            if self.heteroAtoms[pIso].mzOffset < 0:
                                himz = (
                                    peak.mz
                                    + self.heteroAtoms[pIso].mzOffset / peak.loading
                                )  # delta m/z is negative
                            else:
                                himz = (
                                    peak.mz
                                    + peak.xCount * tracer.mzDelta / peak.loading
                                    + self.heteroAtoms[pIso].mzOffset / peak.loading
                                )  # delta m/z is positive

                            if (
                                (himz * (1.0 - self.ppm / 1000000.0))
                                <= cmz
                                <= (himz * (1.0 + self.ppm / 1000000.0))
                            ):
                                dd.append([(cmz, 0), (cmz, cint)])
                ...
                for i in range(len(scan.mz_list)):
                    ...
                    if (peak.mz - 5) < cmz < (dup + 5):
                        dd.append([(cmz, 0), (cmz, cint)])
                        colors.append(
                            Color(47 / 255.0, 79 / 255.0, 79 / 255.0)
                        )  # slategrey
                        strokes.append(0.1)
                        mInt = max(mInt, cint)
            ...
            for iscan in iscans:
                ...
                lp.xValueAxis.valueMax = (
                    peak.mz + peak.xCount * tracer.mzDelta / peak.loading + 5
                )
    ...
    def plotXICForFeature(
        ...
        lp.width = 200
        minDsmall = int(
            max(0, min(int(centerD - 3 * leftFlankD), int(centerD - 3 * leftFlankD)))
        )
        maxDsmall = int(
            min(
    ...
    def writeResultsToPDF(self, mzxml, reportFunction=None):
        ...
        try:
            for chromPeak in SQLSelectAsObject(
                ...
                setattr(chromPeak, "fDesc", loads(base64.b64decode(chromPeak.DSc)))
                setattr(
                    chromPeak, "comments", loads(base64.b64decode(chromPeak.comments))
                )
            ...
            for tracerID, tracer in configTracers.items():
                ...
                for si in range(len(sortedChromPeaks)):
                    ...
                    self.postMessageToProgressWrapper(
                        "text",
                        "%s: Creating PDF (%d features remaining)"
                    ...
                    eicN, times, scanIds, mzsN = mzxml.getEIC(
                        peak.mz, self.chromPeakPPM, filterLine=scanEvent
                    )
                    ...
                    if True:  # Plot of Mass spectra for the feature pair
                        self.plotMSScanForFeaturePair(
                            peak, tracer, scanEvent, mzxml, pdf
                        )
                    ...
                    lines.append(
                        "<u>%s</u> Ion Mode: <u>%s</u>" % (scanEvent, peak.ionMode)
                    )
                    lines.append(
                    ...
                    if peak.artificialEICLShift != 0:
                        lines.append(
                            "Artificial EIC (M') shift: <u>%d</u> scans"
                            % peak.artificialEICLShift
                        )
                    ...
                    if len(peak.heteroIsotopologues) > 0:
                        for hetAtom in peak.heteroIsotopologues:
                            ...
                            for hetAtomCount in pIso:
                                isotopeNumber, element = re.match(
                                    r"([0-9]+)([a-zA-Z]+)", hetAtom, re.I
                                ).groups()
                                ...
                                lines.append(
                                    ...
                                    % (
                                        ...
                                        len(pIso[hetAtomCount]),
                                        100.0
                                        * mean([d[1] for d in pIso[hetAtomCount]]),
                                        100.0
                                        * mean([d[2] for d in pIso[hetAtomCount]]),
                                    )
                                )
                    else:
                    ...
                    p = Paragraph(
                        "<br/>".join(lines), style=getSampleStyleSheet()["Normal"]
                    )
            ...
            if self.writeMZXML & 1:
                ...
                for w in range(len(scans)):
                    ...
                    if not (scanid in newMZXMLData):
                        newMZXMLData[scanid] = Bunch(
                            mzs=[], ints=[], rt=mzxml.getScanByID(scanid).retention_time
                        )
    ...
    def writeIntermediateMZXMLDataToNewMZXMLFile(self, mzxml, newMZXMLData):
        try:
            ...
            if ".mzxml" in self.file.lower():
                toFile = (
                    self.file[0 : self.file.lower().rfind(".mzxml")] + ".proc.mzXML"
                )
                mzxml.resetMZData(self.file, toFile, newMZXMLData)
            elif (
                ".mzml" in self.file.lower() and False
            ):  ## mzml export currently not supported
        ...
        for pol, scanEvent in scanEvents.items():
            if scanEvent != "None":
                ...
                curs.execute(
                    ...
                    % ("MeanScanTime_%s" % pol, str(mean(scanTimes)))
                )
                curs.execute(
                    "INSERT INTO stats(key, value) VALUES('%s', '%s')"
                    % ("SDScanTime_%s" % pol, str(sd(scanTimes)))
                )
                curs.execute(
                    "INSERT INTO stats(key, value) VALUES('%s', '%s')"
                    % ("NumberOfScans_%s" % pol, len(times))
                )
                curs.execute(
                    "INSERT INTO stats(key, value) VALUES('%s', '%s')"
                    % (
                ...
                minInt, maxInt, avgInt = mzxml.getMinMaxAvgSignalIntensities(
                    filterLine=scanEvent
                )
                curs.execute(
                    "INSERT INTO stats(key, value) VALUES('%s', '%s')"
                    % ("MinSignalInt_%s" % pol, minInt)
                )
                curs.execute(
                    "INSERT INTO stats(key, value) VALUES('%s', '%s')"
                    % ("MaxSignalInt_%s" % pol, maxInt)
                )
                curs.execute(
                    "INSERT INTO stats(key, value) VALUES('%s', '%s')"
                    % ("AvgSignalInt_%s" % pol, avgInt)
                )
        ...
        try:
            ...
            if not USEGRADIENTDESCENDPEAKPICKING:
                ...
                self.CP = GradientPeaks(
                    minInt=1000, minIntFlanks=1, minIncreaseRatio=0.01
                )  ## LTQ Orbitrap XL
                self.CP = GradientPeaks(
                    minInt=10000,
                    minIntFlanks=10,
            ...
            if os.path.exists(self.file + getDBSuffix()) and os.path.isfile(
            ...
            self.writeTICsToDB(
                mzxml, {"+": self.positiveScanEvent, "-": self.negativeScanEvent}
            )
            ...
            self.postMessageToProgressWrapper(
                "value", curTracerProgress + 0 * tracerProgressWidth
            )
            self.postMessageToProgressWrapper(
                "text", "%s: Extracting signal pairs" % tracer.name
            )
            ...
            def reportFunction(curVal, text):
                self.postMessageToProgressWrapper(
                    "value",
                    curTracerProgress
                    + 0 * tracerProgressWidth
                    + 0.25 * curVal * tracerProgressWidth,
                )
                self.postMessageToProgressWrapper(
                    "text", "%s: Extracting signal pairs (%s)" % (tracer.name, text)
                )

            mzs, negFound, posFound = self.findSignalPairs(
                curTracerProgress, mzxml, tracer, reportFunction
            )
            self.writeSignalPairsToDB(mzs, mzxml, tracerID)

            self.printMessage(
                "%s: Extracting signal pairs done. pos: %d neg: %d mzs (including mismatches)"
            ...
            self.postMessageToProgressWrapper(
                "value", curTracerProgress + 0.25 * tracerProgressWidth
            )
            self.postMessageToProgressWrapper(
                "text", "%s: Clustering found signal pairs" % tracer.name
            )
            ...
            def reportFunction(curVal, text):
                self.postMessageToProgressWrapper(
                    "value",
                    curTracerProgress
                    + 0.25 * tracerProgressWidth
                    + 0.1 * curVal * tracerProgressWidth,
                )
                self.postMessageToProgressWrapper(
                    "text",
            ...
            self.postMessageToProgressWrapper(
                "value", curTracerProgress + 0.35 * tracerProgressWidth
            )
            self.postMessageToProgressWrapper(
                "text", "%s: Separating feature pairs" % tracer.name
            )
            ...
            def reportFunction(curVal, text):
                self.postMessageToProgressWrapper(
                    "value",
                    curTracerProgress
                    + 0.35 * tracerProgressWidth
                    + 0.3 * curVal * tracerProgressWidth,
                )
                self.postMessageToProgressWrapper(
                    "text", "%s: Separating feature pairs (%s)" % (tracer.name, text)
                )

            chromPeaks = self.findChromatographicPeaksAndWriteToDB(
                mzbins, mzxml, tracerID, reportFunction
            )
            ...
            self.postMessageToProgressWrapper(
                "value", curTracerProgress + 0.65 * tracerProgressWidth
            )
            self.postMessageToProgressWrapper(
                "text", "%s: Removing false positive feature pairs" % tracer.name
            )
            ...
            def reportFunction(curVal, text):
                self.postMessageToProgressWrapper(
                    "value",
                    curTracerProgress
                    + 0.65 * tracerProgressWidth
                    + 0.05 * curVal * tracerProgressWidth,
                )
                self.postMessageToProgressWrapper(
                    "text",
                    "%s: Removing false positive feature pairs (%s)"
                    % (tracer.name, text),
                )
            ...
            self.printMessage(
                "%s: Removing false positive feature pairs done. %d chromatographic peaks"
            ...
            self.postMessageToProgressWrapper(
                "value", curTracerProgress + 0.7 * tracerProgressWidth
            )
            self.postMessageToProgressWrapper(
                "text", "%s: Searching for hetero atoms" % tracer.name
            )
            ...
            def reportFunction(curVal, text):
                self.postMessageToProgressWrapper(
                    "value",
                    curTracerProgress
                    + 0.7 * tracerProgressWidth
                    + 0.05 * curVal * tracerProgressWidth,
                )
                self.postMessageToProgressWrapper(
                    "text", "%s: Annotating feature pairs (%s)" % (tracer.name, text)
                )
            ...
            self.postMessageToProgressWrapper(
                "value", curTracerProgress + 0.75 * tracerProgressWidth
            )
            self.postMessageToProgressWrapper(
                "text", "%s: Grouping feature pairs" % tracer.name
            )
            ...
            def reportFunction(curVal, text):
                self.postMessageToProgressWrapper(
                    "value",
                    curTracerProgress
                    + 0.75 * tracerProgressWidth
                    + 0.05 * curVal * tracerProgressWidth,
                )
                self.postMessageToProgressWrapper(
                    "text", "%s: Grouping feature pairs (%s)" % (tracer.name, text)
                )

            self.groupFeaturePairsUntargetedAndWriteToDB(
                chromPeaks, mzxml, tracer, tracerID, reportFunction
            )
            # endregion
            ...
            self.postMessageToProgressWrapper(
                "value", curTracerProgress + 0.8 * tracerProgressWidth
            )
            self.postMessageToProgressWrapper(
                "text", "%s: Extracting mass spectra to DB" % tracer.name
            )
            ...
            def reportFunction(curVal, text):
                self.postMessageToProgressWrapper(
                    "value",
                    curTracerProgress
                    + 0.8 * tracerProgressWidth
                    + 0.15 * curVal * tracerProgressWidth,
                )
                self.postMessageToProgressWrapper(
                    "text",
            ...
            if self.writeMZXML:
                self.postMessageToProgressWrapper(
                    "value", curTracerProgress + 0.95 * tracerProgressWidth
                )
                self.postMessageToProgressWrapper(
                    "text", "%s: Writing results to mzXML.." % tracer.name
                )

                self.writeResultsToNewMZXMLIntermediateObject(
                    mzxml, newMZXMLData, chromPeaks
                )
            # endregion
            ...
            if self.writeFeatureML:
                self.postMessageToProgressWrapper(
                    "text", "Writing results to featureML.."
                )
            ...
            if self.writePDF:
                ...
                def reportFunction(curVal, text):
                    ...
                    self.postMessageToProgressWrapper(
                        "text", "Writing results to PDF (%s)" % text
                    )
            ...
            if self.writeMZXML:
                self.postMessageToProgressWrapper(
                    "text", "Writing results to new mzXML file"
                )
